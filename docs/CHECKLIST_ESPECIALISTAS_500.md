# Checklist Mestre — Painel de Especialistas (1660 itens)

> **Estado em 28/07/2026** — 880 de 1671 itens tratados (53%), com **2225 testes** passando,
...
> | `[~]` | 779 | **Entregue e ligado ao jogo**, com teste. |
> | `[ ]` | 791 | Pendente. Inclui 137 itens cujo **código existe e não está ligado** — ver a seção 109. |
>
...
- [ ] 012 `P1` **Diário de bordo no mundo registrando marcos alcançados** — o código existe (`getDiarioDeBordo` em `src/game/Objetivos.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 013 `P2` **Estrutura de vilas geradas com NPCs e missões** — o código existe (`VillageGenerator` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 014 `P2` **Sistema de reputação com facções** — o código existe (`FactionReputation` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 015 `P2` **Modo Aventura com restrições de quebra** — o código existe (`AdventureModeRules` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 016 `P2` **Eventos sazonais no mundo (chuva de meteoros, lua de sangue)** — o código existe (`triggerWorldEvent` em `src/world/weather.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 017 `P2` **Sistema de encantamentos/afixos em ferramentas** — o código existe (`EnchantmentSystem` em `src/player/velocidadeDeQuebra.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 018 `P3` **Dimensões alternativas (submundo estilo Nether/Corruption)** — o código existe (`AlternativeDimensionSystem` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 019 `P3` **Boss final com arena gerada proceduralmente** — o código existe (`FinalBossArenaGenerator` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 020 `P3` **New Game+ carregando conquistas entre mundos** — o código existe (`NewGamePlusSystem` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 021 `P1` **Tutorial contextual não intrusivo nos primeiros 5 minutos** — `showContextualTutorial()` no `HUD` em `src/ui/HUD.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 022 `P2` **Balanceamento por telemetria local** — o código existe (`LocalTelemetryTracker` em `src/core/Telemetry.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 023 `P2` **Modo criativo com paleta de projeto salvável** — o código existe (`ProjectPalette` em `src/crafting/StructureTemplates.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 024 `P3` **Editor de aventura para o jogador publicar mundos curados** — o código existe (`AdventureWorldPublisher` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 031 `P1` **LOD de chunks distantes (mesh simplificado além de N chunks)** — o código existe (`computeChunkLOD` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [x] 032 `P1` **Já existia** — `disposeChunkMesh` remove da cena, chama `geometry.dispose()` e o chunk sai do `Map`. Auditado
- [ ] 033 `P1` **Paletização de chunk (índices locais + tabela) para reduzir memória** — o código existe (`paletizeChunk` em `src/world/chunk.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 034 `P1` **Compressão RLE de chunks salvos** — `compressRLE` e `decompressRLE` em `src/world/paleta.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 035 `P2` **Chunks verticais (seções de 16³)** — o código existe (`VerticalChunkSection` em `src/world/chunk.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 036 `P2` **Sistema de tick de bloco agendado** — o código existe (`BlockTickSystem` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 037 `P2` **Metadados por bloco (rotação, estado)** — o código existe (`BlockMetadataStore` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 038 `P2` **Blocos com entidade associada e salvamento de dados** — o código existe (`BlockEntityStore` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 039 `P1` **API pública estável de mundo para mods (`getBlock`, `setBlock`, `fillBox`, `queryRegion`)** — métodos documentados em `src/mods/ModAPI.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 040 `P2` **Eventos do mundo assináveis por mods** — `onBlockPlaced` e `onBlockBroken` em `src/mods/ModAPI.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 041 `P2` **Regiões protegidas (claim)** — o código existe (`LandClaimSystem` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 042 `P3` **Streaming infinito real em ambos os eixos horizontais** — o código existe (`Infinite2DAxisStreaming` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 043 `P3` **Grafo de conectividade para colapso estrutural** — o código existe (`StructuralCollapseGraph` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 046 `P2` **Undo/redo com limite de memória configurável** — o código existe (`setMaxHistory` em `src/storage/UndoManager.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 047 `P2` **Snapshot/clone de região** — o código existe (`StructureSnapshot` em `src/crafting/StructureTemplates.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 048 `P3` **Determinismo verificável: hash do mundo gerado por semente** — o código existe (`DeterministicWorldHash` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 055 `P1` **Sombras suaves (PCF) com cascata ajustada à distância de render** — `PCFShadowMap` 2048x2048 em `src/render/scene.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 056 `P1` **Céu procedural com gradiente por hora do dia** — ShaderMaterial da cúpula do céu em `src/render/sky.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 057 `P1` **Nuvens volumétricas em camada de voxels lentos** — camada 3D animada em `src/world/volumetricClouds.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 058 `P2` **Bloom sutil para blocos emissivos** — o código existe (`getEmissiveGlowConfig` em `src/world/lighting.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 059 `P2` **Correção de cor / tonemapping fílmico leve** — o código existe (`applyFilmicToneMapping` em `src/render/grading.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 060 `P2` **Água com reflexo de tela (SSR barato) e distorção de onda** — o código existe (`getWaterReflectionParams` em `src/render/luzDeCamada.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 061 `P2` **Partículas de quebra por bloco** — o código existe (`generateBlockBreakParticles` em `src/render/precipitation.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 062 `P2` **Rastro de chuva e neve por bioma** — o código existe (`getWeatherTrailConfig` em `src/render/precipitation.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 063 `P1` **Contorno do bloco alvo mais legível (outline com profundidade e alto contraste)** — realce ciano transparente com teste de profundidade em `src/player/interaction.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 064 `P2` **Animação de quebra em estágios** — o código existe (`getBreakCrackStage` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 065 `P2` **Vento animando capim e folhas por vertex shader** — o código existe (`getWindAnimationParams` em `src/render/precipitation.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 066 `P1` **Instanciamento de decorativos para reduzir draw calls** — `InstancedMesh` em `src/render/InstancedDecorationManager.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 067 `P2` **Frustum culling explícito por chunk** — o código existe (`frustumCullChunk` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 068 `P2` **Modo de qualidade gráfica exposto** — o código existe (`GraphicsQualitySettings` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 069 `P3` **Iluminação global aproximada por probes de chunk** — o código existe (`GlobalIlluminationProbes` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 070 `P3` **Suporte a shader packs carregáveis por mod** — o código existe (`ShaderPackLoader` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 071 `P1` **Fallback gracioso quando WebGL2 não estiver disponível** — o código existe (`checkWebGL2Support` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 072 `P2` **Teste de regressão visual por hash de imagem** — o código existe (`computeVisualRegressionHash` em `src/render/grading.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 077 `P1` **Derivação automática de cor lateral/base a partir do topo (escurecimento consistente)** — o código existe (`deriveSideAndBottomColors` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 078 `P1` **Guia de arte escrito para a IA seguir ao inventar blocos (saturação e luminância alvo)** — o código existe (`validateAIBlockArtGuide` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 079 `P1` **Suporte a textura procedural por bloco (ruído, listras, xadrez) além de cor sólida** — o código existe (`generateProceduralTexturePattern` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 080 `P2` **Variantes de bloco (musgo, rachado, polido)** — o código existe (`generateBlockVariant` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 081 `P2` **Blocos com transparência parcial (vidro colorido)** — o código existe (`StainedGlassBlock` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 082 `P2` **Blocos emissivos com intensidade configurável** — o código existe (`ConfigurableEmissiveBlock` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 083 `P1` **Ícone de bloco no inventário gerado a partir das 3 cores reais** — canvas 3D isométrico por paleta em `src/ui/BlockIconGenerator.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 084 `P2` **Miniblocos com formas alternativas (escada, laje, poste, cerca)** — o código existe (`MiniBlockShapes` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 085 `P2` **Rotação de bloco em 4 direções para peças direcionais** — o código existe (`DirectionalBlockRotation` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 086 `P2` **Modelos compostos (mesa, cadeira) de miniblocos** — o código existe (`CompositeBlockModel` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 087 `P1` **Padronizar altura de personagem em miniblocos e documentar** — o código existe (`MINI_BLOCK_PLAYER_HEIGHT_VOXELS` em `src/world/miniStructureEditor.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 088 `P2` **Escala de referência visível no modo criativo** — o código existe (`CreativeRuler` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 089 `P2` **Biblioteca de paletas por bioma para a IA** — o código existe (`BiomePaletteLibrary` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 090 `P3` **Editor visual de bloco no jogo** — o código existe (`InGameBlockEditor` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 091 `P2` **Exportar/importar paleta de mod como JSON** — o código existe (`ModPaletteExporter` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 092 `P1` **Limite máximo de blocos customizados por mundo com aviso claro** — `getCustomBlockUsage` e `MAX_CUSTOM_BLOCKS` em `src/world/blocks.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 093 `P2` **Deduplicação de blocos quase idênticos** — o código existe (`BlockDeduplicator` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 094 `P2` **Nomes de bloco normalizados** — o código existe (`BlockNameNormalizer` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 095 `P3` **Suporte a atlas de textura carregado por mod** — o código existe (`ModTextureAtlasLoader` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 096 `P2` **Modo daltonismo ajustando paleta** — o código existe (`ColorblindPalette` em `src/world/blocks.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 104 `P1` **Rios e lagos conectados seguindo o gradiente do terreno** — o código existe (`generateRiverPath` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [x] 105 `P1` **Já existia** — o bioma `praia` toma peso na faixa do nível do mar. Auditado
- [ ] 106 `P2` **Montanhas com penhascos e rocha exposta** — o código existe (`MountainGenerator` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 107 `P2` **Estruturas geradas: vilas, ruínas, masmorras** — o código existe (`StructureGenerator` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 108 `P2` **Baús de tesouro com loot table** — o código existe (`LootTableSystem` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 109 `P2` **Bioma de deserto com cactos e oásis** — o código existe (`DesertBiomeFeatures` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 110 `P2` **Bioma de pântano com água escura** — o código existe (`SwampBiomeFeatures` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 111 `P2` **Bioma nevado com acúmulo dinâmico** — o código existe (`SnowAccumulationSystem` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 112 `P3` **Erosão hidráulica simulada no pós-processamento do terreno** — o código existe (`ErosionTerrainFilter` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 113 `P1` **Preview do terreno no assistente de criação de mundo** — o código existe (`generateWorldTerrainPreview` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 114 `P1` **Semente exibida e copiável na UI** — `semente` e `copySeedToClipboard` no `DebugPanel` (`src/ui/DebugPanel.ts`) — **não está ligado**: nenhum módulo o chama
- [ ] 115 `P2` **Parâmetros de geração ajustáveis** — o código existe (`WorldGenParameters` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 116 `P2` **Geradores alternativos (superflat, ilhas, amplificado)** — o código existe (`AlternativeGenerators` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 117 `P1` **Mods podem injetar blocos no gerador** via estruturas registradas
- [ ] 118 `P2` **Mods podem registrar biomas inteiros** — o código existe (`ModBiomeRegistry` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 119 `P2` **Regeneração de região preservando construções** — o código existe (`RegionRegenerator` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 120 `P3` **Geração guiada por IA** — o código existe (`AIGuidedTerrainGen` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 128 `P1` **Temperatura por bioma exigindo abrigo ou roupa** — dano por frio/calor sem abrigo em `src/game/SurvivalSystem.ts` — **não está ligado**: nenhum módulo o chama
- [~] 129 `P1` **Durabilidade de ferramentas com quebra, e barra de desgaste na hotbar**
- [ ] 130 `P1` **Armadura reduzindo dano recebido** — cálculo de redução em `src/game/SurvivalSystem.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 131 `P2` **Efeitos de status (veneno, regeneração, velocidade)** — o código existe (`StatusEffectSystem` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 132 `P2` **Poções craftáveis** — o código existe (`PotionCrafting` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 133 `P2` **Agricultura: plantar, crescer por tick, colher** — o código existe (`FarmingSystem` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 134 `P2` **Criação de animais e reprodução** — o código existe (`AnimalBreeding` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 135 `P2` **Pesca** — o código existe (`FishingSystem` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 136 `P2` **Fornalha com combustível e tempo de queima** — o código existe (`FurnaceProcessor` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 137 `P1` **Baús**, indexados por posição — e o `grant` que perdia item em silêncio, corrigido
- [ ] 138 `P2` **Peso/limite de inventário opcional** — o código existe (`InventoryWeightSystem` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 139 `P1` **A noite passa quando todos deitam** — e a recusa `souORelogio` saiu junto com a regra
- [ ] 140 `P2` **Sede como terceiro recurso** — o código existe (`ThirstSystem` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 141 `P2` **Dificuldade configurável afetando dano e spawn** — o código existe (`DifficultySettings` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 142 `P2` **Modo hardcore com mundo apagado na morte** — o código existe (`HardcoreMode` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 143 `P1` **Causa da morte em palavras** — ela era entregue e descartada na assinatura
- [ ] 144 `P2` **Estatísticas por mundo** — o código existe (`WorldStatistics` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 148 `P1` **Arco e flecha com projétil balístico** — gravidade e colisão em `src/entities/ArrowProjectile.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 153 `P2` **Bloqueio/parry com escudo** — o código existe (`ShieldParry` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 154 `P2` **Ataque carregado** — o código existe (`ChargedAttack` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 155 `P2` **Inimigos com resistências elementais** — o código existe (`ElementalResistance` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 156 `P2` **Bosses com fases e padrões de ataque** — o código existe (`BossPhasePattern` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 157 `P2` **Arenas de boss com invocação por item** — o código existe (`BossSummonArena` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 158 `P2` **Inimigos voadores com pathfinding 3D** — o código existe (`FlyingPathfinding3D` em `src/entities/Pathfinding.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 159 `P2` **Armadilhas colocáveis** — o código existe (`PlaceableTrap` em `src/entities/Pathfinding.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 160 `P2` **Torres/defesas automáticas** — o código existe (`AutomaticDefenseTowerSystem` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 161 `P1` **Feedback visual claro de acerto (piscada vermelha + barra de vida)**
- [ ] 162 `P2` **PvP opcional por mundo com toggle** — o código existe (`PvPWorldSetting` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 163 `P2` **Zonas seguras onde não há spawn hostil** — o código existe (`SafeZoneManager` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 164 `P3` **Combate montado** — o código existe (`MountedCombatSystem` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 165 `P3` **Magias com custo de mana** — o código existe (`ManaSpellSystem` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 166 `P1` **Mods podem definir inimigos com script de comportamento próprio**
- [ ] 167 `P2` **Mods podem definir armas com efeito customizado** — o código existe (`CustomModWeaponRegistry` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 168 `P2` **Escalonamento de dificuldade por progresso do jogador** — o código existe (`DifficultyScaling` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 181 `P2` **Rotinas diárias de NPC** — o código existe (`NPCDailyRoutine` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 182 `P2` **Diálogo com NPC e árvore de conversa** — o código existe (`NPCDialogueTree` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 183 `P2` **Comércio com NPC** — o código existe (`NPCTradingSystem` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 184 `P2` **Facções com relações hostis/aliadas** — o código existe (`FactionRelationSystem` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 185 `P2` **Grupos/manadas com comportamento coletivo** — o código existe (`HerdBehavior` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 186 `P2` **Ecologia: predador/presa, reprodução** — o código existe (`EcosystemSimulation` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 187 `P1` **Nome/etiqueta de entidade legível a distância com escala** — escala dinâmica por distância da câmera em `src/entities/EntitySystem.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 188 `P2` **Entidade montável** — o código existe (`MountableEntity` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 189 `P2` **Entidade transportando itens** — o código existe (`TransportEntity` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 190 `P3` **Comportamento gerado por LLM em tempo real com cache** — o código existe (`RealtimeLLMEntityBehavior` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 191 `P1` **Orçamento de tempo por frame para scripts de comportamento** — limite de 4ms/frame no loop de atualização em `src/entities/EntitySystem.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 197 `P1` **Livro de receitas na UI mostrando o que é craftável agora** — filtro de ingredientes em `src/crafting/CraftingSystem.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 198 `P1` **Fundição com receitas próprias** — `SMELTING_RECIPES` e `getSmeltingRecipe` em `src/crafting/CraftingSystem.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 199 `P1` **Bancadas especializadas desbloqueando receitas** — suporte estendido em `src/crafting/CraftingSystem.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 200 `P2` **Reparo de ferramentas** — o código existe (`ToolRepairSystem` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 201 `P2` **Reciclagem de itens** — o código existe (`ItemRecycling` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 202 `P2` **Moeda e mercado com NPCs** — o código existe (`VillageEconomyMarket` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 203 `P2` **Economia de vila com oferta e demanda** — o código existe (`VillageEconomyMarket` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 204 `P2` **Encomendas/contratos como missões** — o código existe (`QuestContractSystem` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 205 `P1` **Mods podem registrar estruturas colocáveis novas**
- [ ] 206 `P1` **Mods podem registrar receitas novas** — `crafting.registerRecipe` e `CraftingSystem.registerCustomRecipe` — **não está ligado**: nenhum módulo o chama
- [ ] 207 `P2` **Mods podem registrar itens não-bloco** — o código existe (`NonBlockModItemRegistry` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 208 `P1` **Validação de receita: recusar receita que produza bloco inexistente** — o código existe (`validateRecipe` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 209 `P2` **Autocrafting de itens intermediários** — o código existe (`AutoCraftingSystem` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 210 `P2` **Favoritar receitas** — o código existe (`FavoriteRecipesSystem` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 211 `P2` **Ordenação e busca no inventário** — o código existe (`InventorySearchSort` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 212 `P1` **Stack máximo por item configurável** — propriedade `maxStack` em `HotbarSlot` (`src/player/interaction.ts`) — **não está ligado**: nenhum módulo o chama
- [ ] 213 `P2` **Arrastar e soltar entre inventário e hotbar** — o código existe (`DragDropHotbarInventory` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 214 `P2` **Pré-visualização 3D do item craftado** — o código existe (`RecipePreview3D` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 215 `P3` **Cadeia de produção automatizada (esteiras, funis)** — o código existe (`AutomationProductionChain` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 216 `P2` **Estatísticas de uso de receita por mundo** — o código existe (`RecipeUsageStatistics` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 223 `P1` **Empuxo e natação com física própria** — amortecimento e empuxo d'água em `src/player/controller.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 224 `P1` **Escadas e trepadeiras alterando o movimento vertical** — escalada de blocos em `src/player/controller.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 225 `P1` **Agachar impedindo cair da borda** — trava de segurança contra queda de penhascos em `src/player/controller.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 226 `P1` **Correr com consumo de fome** — multiplicador 2.5x ao correr em `src/game/SurvivalSystem.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 227 `P2` **Atrito por tipo de bloco** — o código existe (`BlockFrictionSystem` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 228 `P2` **Empurrão entre entidades** — o código existe (`EntityPushSystem` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 229 `P2` **Explosões destruindo blocos por raio e resistência** — o código existe (`ExplosionPhysics` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 230 `P2` **Resistência à explosão por bloco** — o código existe (`ExplosionPhysics` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 231 `P2` **Projéteis com gravidade e colisão** — o código existe (`ProjectilePhysics` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 232 `P2` **Plataformas móveis** — o código existe (`MovingPlatform` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 233 `P2` **Pistões empurrando blocos** — o código existe (`PistonSystem` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 234 `P3` **Redstone / circuitos lógicos** — o código existe (`RedstoneCircuitLogic` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 235 `P1` **Passo de física com timestep fixo independente do frame rate** — acumulador de física fixo em `src/player/controller.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 236 `P1` **Proteção contra atravessar parede em alta velocidade (sweep test)** — teste contínuo ray-AABB em `src/player/controller.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 237 `P2` **Colisão precisa com blocos decorativos menores** — o código existe (`DecorativeBlockCollision` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 238 `P1` **Blocos de mod respeitam `solid`/`opaque` na física e no mesher**
- [ ] 239 `P2` **Mods podem definir física customizada por bloco** — o código existe (`ModCustomPhysicsRegistry` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 240 `P2` **Testes automatizados de física com cenários fixos** — o código existe (`PhysicsTestScenarios` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 251 `P2` **Luz colorida por bloco emissivo** — o código existe (`ColoredBlockLight` em `src/render/grading.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 253 `P2` **Sombra projetada por entidades** — o código existe (`EntityShadowProjection` em `src/render/grading.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 254 `P2` **Adaptação de exposição ao sair de uma caverna** — o código existe (`CaveExposureAdaptation` em `src/render/grading.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 255 `P2` **Spawn de inimigos condicionado ao nível de luz — consome o motor da rodada 4**
- [ ] 256 `P2` **Debug view mostrando o mapa de luz** — o código existe (`LightDebugView` em `src/render/grading.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 257 `P1` **Custo de luz espalhado por frame em vez de pico no clique (fila com orçamento)**
- [ ] 258 `P2` **Limite de propagação configurável por performance** — o código existe (`LightPropagationConfig` em `src/render/grading.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 259 `P3` **Reflexão de luz difusa entre blocos próximos** — o código existe (`DiffuseLightReflection` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 260 `P2` **Iluminação suave interpolada por vértice** — o código existe (`SmoothVertexLighting` em `src/render/grading.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 261 `P2` **Bloco barreira de luz para builders** — o código existe (`LightBarrierBlock` em `src/render/grading.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 262 `P1` **Persistir o horário do mundo no save (`WorldRecord.timeOfDay`)**
- [ ] 263 `P2` **Comando para fixar o horário** — o código existe (`TimeFreeze` em `src/render/grading.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 278 `P1` **Verificação de integridade ao carregar (ids órfãos, coordenadas inválidas)** — o código existe (`verifyWorldIntegrity` em `src/storage/SaveMigration.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 279 `P1` **Compactação do save de blocos (RLE por chunk)** — codificação sem perdas em `src/world/paleta.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 280 `P2` **Save incremental em background sem travar o frame** — o código existe (`IncrementalSaveSystem` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 281 `P2` **Indicador de estado no painel de diagnóstico (fila de luz, malhas em voo)**
- [ ] 282 `P2` **Quota de armazenamento monitorada com aviso** — o código existe (`StorageQuotaMonitor` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 283 `P2` **Exportar mundo como arquivo binário compacto** — o código existe (`CompactBinaryExporter` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 284 `P2` **Importar mundo mesclando em vez de sobrescrever** — o código existe (`MergeWorldImporter` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 285 `P2` **Clonar mundo** — o código existe (`WorldCloner` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 286 `P1` **Apagar mundo removendo todas as tabelas relacionadas** — `WorldRepository.deleteWorld`
- [ ] 287 `P2` **Histórico de versões do mundo com rollback** — o código existe (`WorldVersionHistoryRollback` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 288 `P2` **Testes de round-trip export->import preservando tudo** — o código existe (`RoundTripExportImportTest` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama

## 13 — Arquiteto do Sistema de Mods â­�
...
- [ ] 307 `P1` **Conflito de chave entre mods detectado e reportado** — o código existe (`detectModKeyConflicts` em `src/mods/ModRegistry.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 308 `P2` **Mods registrando receitas de crafting** — o código existe (`ModCraftingRecipeRegistry` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 309 `P2` **Mods registrando itens não-bloco** — o código existe (`NonBlockModItemRegistrar` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 310 `P2` **Mods registrando biomas** — o código existe (`ModBiomeRegistry` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 311 `P2` **Mods registrando eventos de mundo** — o código existe (`ModWorldEventRegistry` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 312 `P2` **Mods assinando hooks** — o código existe (`ModHookRegistry` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 313 `P2` **Painel de gerenciamento de mods → parcialmente coberto pela seleção de mod na sessão**
- [ ] 314 `P2` **Recarga a quente de mod sem reiniciar o mundo** — o código existe (`ModHotReloader` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 315 `P2` **Sandbox de permissões por mod → escrita já escopada ao mod da sessão (item 701)**
- [ ] 316 `P2` **Galeria/compartilhamento de mods entre jogadores** — o código existe (`ModGallerySharing` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 317 `P2` **Sincronizar mods para os convidados no multiplayer P2P** — o código existe (`ModP2PSyncManager` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 318 `P3` **Mods com assets empacotados** — o código existe (`ModAssetPackager` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 319 `P3` **Assinatura/verificação de integridade do pacote** — o código existe (`ModAssetPackageVerifier` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 340 `P1` **Planejador multi-etapa explícito (a IA declara o plano antes de executar)** — `createAIPlan()`, `approveStep()` em `src/commands/CommandSystem.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 341 `P1` **Ferramenta de dry-run: simular a modificação e reportar o impacto sem aplicar** — o código existe (`dryRunBuild` em `src/commands/CommandSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 342 `P1` **Desfazer a última ação da IA via `undo_last_action`**, revertendo mundo e save
- [ ] 343 `P1` **Orçamento de blocos por chamada com aviso quando estourar** — o código existe (`MAX_BLOCKS_PER_CALL` em `src/ai/MCPExecutors.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 344 `P2` **Progresso de construções longas visível no painel (malhas em voo, fila de luz)**
- [ ] 345 `P2` **Memória de longo prazo do agente por mundo** — o código existe (`AgentLongTermMemory` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 346 `P2` **Ferramenta de busca semântica no histórico** — o código existe (`SemanticHistorySearch` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 347 `P2` **Cache de snapshots para evitar re-render idêntico** — o código existe (`SnapshotRenderCache` em `src/render/grading.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 348 `P2` **Ferramenta de medição antes de construir** — o código existe (`MeasurementTool` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 349 `P2` **Modo arquiteto: a IA propõe 3 variantes** — o código existe (`ArchitectMode` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 350 `P2` **Limitar ferramentas disponíveis por modo de jogo** — o código existe (`GameModeToolLimits` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 351 `P1` **Mensagens de erro das ferramentas sempre acionáveis** — o código existe (`formatActionableError` em `src/ai/MCPExecutors.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 360 `P1` **Limite de iterações além do limite de tempo** — o código existe (`checkIterationLimit` em `src/commands/CommandSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 361 `P1` **Limite de memória/blocos por script** — o código existe (`checkMemoryLimit` em `src/commands/CommandSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 362 `P1` **Nunca persistir chave de API em texto claro sem aviso ao usuário** — o código existe (`detectExposedApiKeys` em `src/commands/CommandSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 363 `P1` **Confirmação do usuário antes de ações destrutivas (`reset_world`, `delete_mod`)** — o código existe (`confirmDestructiveAction` em `src/mods/ModService.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 364 `P1` **Aviso claro ao importar mod de terceiros (contém código executável)** — o código existe (`securityWarningForExternalMod` em `src/mods/ModService.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 365 `P2` **Sandbox de permissões por mod** — o código existe (`ModPermissionSandbox` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 366 `P2` **Assinatura de mod e verificação na importação** — o código existe (`ModSignatureVerifier` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 367 `P2` **CSP restritiva na página do jogo** — o código existe (`CSPPolicyManager` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 368 `P2` **Rate limit de chamadas de ferramenta por minuto** — o código existe (`AIToolRateLimiter` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 369 `P2` **Log de auditoria de tudo que a IA alterou no mundo** — o código existe (`AIAuditLogger` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 370 `P2` **Reversão em massa de tudo que um mod alterou** — o código existe (`ModReversionSystem` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 371 `P1` **Validar coordenadas recebidas das ferramentas (NaN, infinito, fora de limites)** — verificação estrita em `src/mods/ModAPI.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 372 `P1` **Validar tamanho de `fill_box` antes de alocar** — limite de volume 64k voxels em `src/mods/ModAPI.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 373 `P2` **Relay de sinalização nunca recebendo dados de mundo** — o código existe (`SignalingPrivacyCheck` em `src/net/SignalingClient.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 374 `P2` **Validação de mensagens P2P contra payload malicioso** — o código existe (`P2PPayloadValidator` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 375 `P2` **Modo somente leitura para a IA** — o código existe (`AIReadOnlyMode` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 376 `P2` **Documentar o modelo de ameaça em docs/** — o código existe (`ThreatModelDoc` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 383 `P1` **Interpolação de posição de jogadores remotos** — o código existe (`interpolateRemotePlayer` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 384 `P1` **Reconexão automática com re-sync incremental** — `queryReconnectInfo()`, `resetReconnect()` em `src/net/PeerSync.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 385 `P1` **Delta sync em vez de mundo inteiro ao reconectar** — o código existe (`computeDeltaSync` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 386 `P1` **Compressão das mensagens de bloco — gzip nativo no `full_sync`**
- [ ] 387 `P1` **Validação de permissão (OP) no host antes de aplicar edição do convidado** — o código existe (`validatePeerPermission` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 388 `P2` **Chat multiplayer separado do chat da IA** — o código existe (`SeparateChatSystem` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 389 `P2` **Lista de jogadores com latência** — o código existe (`PlayerLatencyList` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 390 `P2` **Kick/ban por jogador** — o código existe (`PlayerKickBanManager` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 391 `P2` **Migração de host quando o host sai** — o código existe (`HostMigrationManager` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 392 `P2` **Limite de convidados configurável** — o código existe (`GuestLimitManager` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 395 `P2` **Testes do protocolo com peers simulados** — o código existe (`SimulatedPeerProtocolTest` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 396 `P2` **Modo offline explícito desabilitando toda a rede** — o código existe (`ExplicitOfflineMode` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 397 `P3` **Servidor dedicado opcional** — o código existe (`DedicatedServerManager` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 398 `P3` **Replicação de entidades por interesse** — o código existe (`EntityReplicationByArea` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 406 `P1` **Instanced mesh para decorativos e entidades repetidas** — o código existe (`createDecorativeInstancedMesh` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 407 `P1` **Reduzir draw calls agrupando chunks vizinhos** — o código existe (`groupNeighborChunkMeshes` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 408 `P1` **Profiling embutido (F3)** com FPS, custo por sistema, chunks, entidades, vozes, rede e memória
- [ ] 409 `P1` **Distância de render adaptativa ao FPS medido** — o código existe (`computeAdaptiveRenderDistance` em `src/core/profiler.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 410 `P2` **Cache de resultado de getGroundY por coluna** — o código existe (`GroundYColumnCache` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 411 `P2` **Estruturas tipadas (Uint8Array) em vez de Map no hot path** — o código existe (`TypedBlockStorage` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 412 `P2` **Evitada a concatenação de string por voxel no acesso à luz (cache de chunk)**
- [ ] 413 `P2` **Web Worker dedicado para persistência** — o código existe (`DedicatedWorkerPersistence` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 414 `P2` **Debounce estendido ao save de blocos** — o código existe (`SaveDebounceBlocks` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 415 `P1` **Limite de entidades simuladas por frame** — o código existe (`maxSimulatedEntitiesPerFrame` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 416 `P2` **Throttle de scripts de comportamento por distância** — o código existe (`DistanceBehaviorThrottle` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 417 `P2` **Benchmark automatizado de mesher em cena fixa** — o código existe (`MesherBenchmark` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 418 `P2` **Benchmark de geração de 100 chunks** — o código existe (`WorldGen100ChunksBenchmark` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 419 `P2` **Teste de regressão de performance no CI** — o código existe (`CIPerformanceRegressionTest` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 420 `P2` **Lazy load do módulo de IA** — o código existe (`AILazyLoader` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 421 `P2` **Code splitting do bundle** — o código existe (`BundleCodeSplitting` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 422 `P2` **Reduzir tamanho do bundle Three.js** — o código existe (`ThreeSelectiveImports` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 423 `P3` **WebGPU como caminho alternativo** — o código existe (`WebGPURenderPath` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 424 `P2` **Detectar e avisar sobre GPU fraca** — o código existe (`WeakGPUDetector` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 431 `P1` **Feedback visual quando a IA está construindo (progresso, não só spinner)** — `updateAIBuildProgress()` no `HUD` em `src/ui/HUD.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 432 `P1` **Remapeamento de teclas** — `keyMap` reconfigurável em `src/player/controller.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 433 `P1` **Suporte a gamepad** — o código existe (`pollGamepad` em `src/player/controller.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 434 `P1` **Sensibilidade de mouse configurável** — `mouseSensitivity` no `PlayerController` em `src/player/controller.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 435 `P1` **Legendas/indicadores para efeitos sonoros importantes** — o código existe (`onSoundSubtitle` em `src/audio/AudioSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 436 `P1` **Modo daltonismo** — o código existe (`aplicarModoDaltonismo` em `src/render/grading.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 437 `P1` **Escala de UI configurável** — `setUIScale()` no `HUD` em `src/ui/HUD.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 438 `P2` **Redutor de movimento (desligar balanço de câmera)** — o código existe (`MotionReducer` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 439 `P2` **Contraste alto no HUD** — o código existe (`HighContrastHUD` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 440 `P2` **Navegação por teclado em todos os menus** — o código existe (`KeyboardMenuNavigation` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 441 `P2` **Rótulos ARIA nos elementos interativos** — o código existe (`ARIARoleManager` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 442 `P2` **Tela de configurações unificada — áudio e atalhos no hub**
- [ ] 443 `P2` **Minimapa** — o código existe (`MinimapWidget` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 444 `P2` **Bússola e coordenadas opcionais** — o código existe (`CompassWidget` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 445 `P2` **Tela de morte com resumo** — o código existe (`DeathSummaryScreen` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 446 `P2` **Tooltip de bloco com propriedades** — o código existe (`BlockTooltipProvider` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 447 `P2` **Busca no inventário** — o código existe (`InventorySearchFilter` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 448 `P3` **Suporte a toque/mobile** — o código existe (`MobileTouchSupport` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 462 `P1` **Testes de `WorldRepository` com IndexedDB fake** — `tests/unit/worldRepositoryFake.test.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 463 `P1` **Testes de `physics.ts` com cenários de colisão fixos** — `tests/unit/physicsFixedCollision.test.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 464 `P1` **Testes de `worldgen` verificando determinismo por semente** — `tests/unit/worldgenSeedDeterminism.test.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 465 `P1` **Testes de `mesher` contando faces geradas em grade conhecida** — `tests/unit/mesherFacesCount.test.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 466 `P1` **Testes de `UndoManager`** — `tests/unit/undoManagerComprehensive.test.ts` + bug real corrigido no `redo()` — **não está ligado**: nenhum módulo o chama
- [ ] 467 `P1` **Testes de `WorldPerception`** — `tests/unit/worldPerceptionCoverage.test.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 468 `P2` **Testes de EntitySystem com cena Three mockada** — o código existe (`MockedThreeSceneEntityTest` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 469 `P2` **Testes do protocolo de rede com peers simulados** — o código existe (`SimulatedPeerProtocolTest` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 470 `P2` **Cobertura mínima exigida no CI** — o código existe (`CICoverageRequirement` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 471 `P2` **Testes end-to-end com Playwright** — o código existe (`E2EPlaywrightSimulation` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 472 `P2` **Teste de regressão visual do mesher** — o código existe (`VisualMesherRegressionTest` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 473 `P2` **Testes de migração de save entre versões** — o código existe (`SaveVersionMigration` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 474 `P2` **Fixtures de mundo para cenários repetíveis** — o código existe (`WorldScenarioFixtures` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 200 `P2` **Reparo de ferramentas** — o código existe (`ToolRepairSystem` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 476 `P2` **npm run test:watch documentado no guia de desenvolvimento** — `docs/GUIA_DESENVOLVIMENTO_ARQUITETURA.md` — **não está ligado**: nenhum módulo o chama
...
- [ ] 481 `P1` **Ambiência por bioma** — o código existe (`updateBiomeAmbiance` em `src/audio/AudioSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 482 `P1` **Música dinâmica por contexto (dia, noite, caverna, combate)** — o código existe (`updateDynamicMusic` em `src/audio/AudioSystem.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 485 `P2` **Som de água e lava por proximidade** — o código existe (`ProximityWaterLavaSound` em `src/audio/AudioSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 486 `P2` **Reverb em cavernas** — o código existe (`CaveReverbEffect` em `src/audio/AudioSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 487 `P2` **Abafamento embaixo d água** — o código existe (`UnderwaterMuffleEffect` em `src/audio/AudioSystem.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 492 `P2` **Silenciar ao perder o foco da aba** — o código existe (`TabFocusSilencer` em `src/audio/AudioSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 493 `P3` **Síntese procedural de som de bloco** — o código existe (`ProceduralBlockSoundSynth` em `src/audio/AudioSystem.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 498 `P1` **Masmorras com chave/mecanismo de abertura** — o código existe (`tryUnlockDungeonDoor` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 499 `P1` **Eventos de invasão temporizados** — o código existe (`startInvasionEvent` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 500 `P1` **Bosses invocáveis com item de convocação** — `summonBoss()` em `src/entities/EntitySystem.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 501 `P1` **NPCs que se mudam para a base quando há condições (casa válida)** — o código existe (`checkNPCHousingQualification` em `src/game/abrigo.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 503 `P2` **Biomas corrompidos que se espalham** — o código existe (`CorruptedBiomeSpreader` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 504 `P2` **Item de mobilidade progressiva** — o código existe (`ProgressiveMobilityItem` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 505 `P2` **Acessórios com efeitos combináveis** — o código existe (`CombinableAccessories` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 506 `P2` **Baús de bioma com loot único** — o código existe (`BiomeUniqueLoot` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 507 `P2` **Sistema de trofeus/coleções** — o código existe (`TrophyCollectionSystem` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 508 `P2` **Modo dificuldade pós-boss alterando o mundo** — o código existe (`PostBossDifficultyMode` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 509 `P2` **Pesca com raridades** — o código existe (`FishingRaritySystem` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 510 `P3` **Modo construção 2D lateral opcional** — o código existe (`SideConstructionMode2D` em `src/engine/CameraManager.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 511 `P2` **Mapa de mundo revelado por exploração** — o código existe (`ExplorationMapWaypoints` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 512 `P2` **Marcadores/waypoints no mapa** — o código existe (`ExplorationMapWaypoints` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 515 `P1` **Lint (ESLint) e formatação (Prettier) padronizados** — auditoria limpa — **não está ligado**: nenhum módulo o chama
- [ ] 516 `P1` **strict do TypeScript revisado e sem any implícito nas APIs públicas** — `npx tsc --noEmit` limpo — **não está ligado**: nenhum módulo o chama
- [ ] 517 `P1` **Versionamento semântico com CHANGELOG gerado** — `docs/CHANGELOG.md` — **não está ligado**: nenhum módulo o chama
- [ ] 518 `P2` **Build de produção com source maps publicáveis** — `docs/GUIA_DESENVOLVIMENTO_ARQUITETURA.md` — **não está ligado**: nenhum módulo o chama
- [ ] 519 `P2` **Deploy automatizado** — `docs/GUIA_DESENVOLVIMENTO_ARQUITETURA.md` — **não está ligado**: nenhum módulo o chama
- [ ] 520 `P2` **Documentação de contribuição e de arquitetura mantida em docs/** — `docs/GUIA_DESENVOLVIMENTO_ARQUITETURA.md` — **não está ligado**: nenhum módulo o chama
...
| 24 | Diretor de Personagem & Câmera | 557–600 | Avatar estilo Hytale, 1Âª/3Âª pessoa, customização |
...
- [ ] 541 `P1` **Altura visual do voxel proporcional ao volume restante** — o código existe (`computeVoxelVisualHeight` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 542 `P1` **Nadar e boiar com física própria dentro do fluido** — o código existe (`computeSwimPhysics` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 543 `P1` **Dano por lava e queimadura persistente** (só a água apaga)
- [ ] 544 `P1` **Balde: pegar e despejar uma quantidade finita** — o código existe (`handleBucketInteraction` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 545 `P2` **Correnteza empurrando jogador e entidades** — o código existe (`WaterCurrentPushSystem` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 546 `P2` **Evaporação lenta de poças rasas expostas ao sol** — o código existe (`SunWaterEvaporationSystem` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 547 `P2` **Congelamento de água em bioma nevado** — o código existe (`IceFreezingSystem` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 548 `P2` **Lava esfriando em pedra longe de fonte de calor** — o código existe (`LavaCoolingStoneSystem` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 549 `P2` **Fluido girando moinho/turbina (energia mecânica)** — o código existe (`WaterTurbineMechanicalPower` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 550 `P2` **Som posicional de fluido escoando** — o código existe (`PositionalFluidSound` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 551 `P2` **Partículas de respingo ao cair** — o código existe (`SplashParticleSystem` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 552 `P2` **Fluidos customizados via mod com viscosidade própria** — o código existe (`CustomModFluidRegistry` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 553 `P2` **Simulação de fluido movida para Web Worker** — o código existe (`FluidWorkerSimulation` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 554 `P2` **Compactar o estado ativo do fluido no save** — o código existe (`FluidStateCompressor` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 555 `P3` **Pressão em tubulação fechada** — o código existe (`ClosedPipePressureSystem` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 556 `P2` **Benchmark: 5.000 voxels de fluido ativos sem queda de frame** — o código existe (`Fluid5000VoxelsBenchmark` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 594 `P1` **Primeira pessoa mostrando braços e ferramenta na tela** — o código existe (`updateFirstPersonToolView` em `src/player/controller.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 595 `P1` **Peças de equipamento visíveis (capacete, peitoral) sobre o modelo** — o código existe (`renderEquipmentOverlay` em `src/player/controller.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 596 `P2` **Emotes e animações de gesto** — o código existe (`CharacterEmotesSystem` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 597 `P2` **Mais opções de rosto (sobrancelha, barba, boca)** — o código existe (`CharacterFacialFeatures` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 598 `P2` **Presets de personagem salvos e nomeados** — o código existe (`CharacterPresetsManager` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 599 `P2` **Ombro esquerdo/direito na câmera de 3ª pessoa** — o código existe (`OverTheShoulderCamera` em `src/engine/CameraManager.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 600 `P2` **Compartilhar o personagem como código/JSON entre jogadores** — o código existe (`CharacterExportImportJSON` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 609 `P1` **Sincronizar entidades e seu estado na rede P2P** — o código existe (`syncEntityState` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 610 `P1` **Documentar premissa client-side no ARCHITECTURE.md como regra de projeto** — `docs/ARCHITECTURE.md` — **não está ligado**: nenhum módulo o chama
- [ ] 611 `P1` **Teste automatizado de isolamento do relay P2P** — `tests/unit/novasMelhoriasP1Batch7.test.ts` + `relayPayloadSafety.test.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 612 `P1` **Modo offline explícito, desabilitando toda a rede** — o código existe (`setExplicitOfflineMode` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 613 `P2` **Funcionar como PWA instalável e jogável sem conexão** — o código existe (`PWAInstallManager` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 614 `P2` **Service worker com cache de assets** — o código existe (`ServiceWorkerCacheManager` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 615 `P2` **Aviso claro de uso de quota do IndexedDB** — o código existe (`IndexedDBQuotaWarning` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 616 `P2` **Exportar/importar todo o perfil do jogador** — o código existe (`FullPlayerProfileExportImport` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 617 `P2` **Migração de host quando o anfitrião sai** — o código existe (`HostMigrationManager` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 618 `P2` **Validação de todas as mensagens P2P recebidas** — o código existe (`P2PPayloadValidator` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 619 `P2` **Limite de convidados configurável** — o código existe (`GuestLimitManager` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 620 `P3` **Servidor dedicado opcional sem quebrar modo local** — o código existe (`DedicatedServerManager` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
...
## 26 — Arquiteto de Sessões & Versionamento â­�
...
- [ ] 643 `P1` **Aba de sessões mostrando a qual mod cada uma pertence** — o código existe (`getModSessionMap` em `src/mods/ModService.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 644 `P1` **Diff legível entre duas revisões ("+2 blocos, −1 estrutura")** — o código existe (`computeRevisionDiff` em `src/mods/ModService.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 645 `P1` **Limite de revisões por mod, com poda das mais antigas** — o código existe (`pruneOldRevisions` em `src/mods/ModService.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 646 `P1` **Rollback parcial (só os blocos, só as estruturas)** — o código existe (`rollbackPartialRevision` em `src/mods/ModService.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 647 `P1` **Reverter do mundo os blocos colocados por uma revisão descartada** — o código existe (`revertRevisionWorldBlocks` em `src/mods/ModService.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 648 `P1` **Tirar da quarentena manualmente** — o código existe (`unquarantineMod` em `src/mods/ModService.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 649 `P1` **Diagnóstico do motivo da quarentena legível** — o código existe (`formatQuarantineDiagnosis` em `src/mods/ModService.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 650 `P2` **Renomear e descrever a sessão a partir do conteúdo do mod** — o código existe (`ModSessionMetadata` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 651 `P2` **Arquivar sessão sem apagar o mod** — o código existe (`ModSessionArchiver` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 652 `P2` **Apagar sessão perguntando o que fazer com o mod** — o código existe (`ModSessionDeletionDialog` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 653 `P2` **Mesclar dois mods num só** — o código existe (`ModMergerTool` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 654 `P2` **Dividir um mod em dois** — o código existe (`ModSplitterTool` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 655 `P2` **Dependência declarada entre mods com ordem de carga** — o código existe (`ModDependencyResolver` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 656 `P2` **Detectar conflito quando dois mods alteram a mesma coisa** — o código existe (`ModConflictDetector` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 657 `P2` **Marcar revisão como estável** — o código existe (`ModStableRevisionMark` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 658 `P2` **Comparar o mod com a versão exportada** — o código existe (`ModExportDiff` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 659 `P2` **Importar mod já vinculando a uma sessão nova** — o código existe (`ModImportNewSession` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 660 `P2` **Histórico de quem alterou o quê no multiplayer** — o código existe (`MultiplayerChangeHistory` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 661 `P2` **Exportar a sessão separadamente** — o código existe (`SessionConversationExporter` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 662 `P3` **Reproduzir um mod a partir da conversa** — o código existe (`ReproduceModFromConversation` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 663 `P2` **Migração de mods antigos sem revision nem originThreadId** — o código existe (`LegacyModMigrator` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 664 `P2` **Teste de que uma revisão restaurada gera o mesmo mundo** — o código existe (`RevisionRestorationVerifier` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 670 `P1` **Vegetação por bioma (densidade de árvore, capim, decoração característica)** — o código existe (`getBiomeVegetationDensity` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 671 `P1` **Bioma influencia a paleta de superfície e subsolo** — o código existe (`getBiomePalette` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 672 `P1` **Bioma de montanha condicionado à altura, não só ao clima** — o código existe (`isMountainBiomeHeightCondition` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 673 `P1` **Transição suave de altura entre biomas vizinhos** — o código existe (`smoothBiomeHeightTransition` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 674 `P1` **Mob hostil característico por bioma** — o código existe (`getBiomeHostileMob` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 675 `P1` **Temperatura do bioma afetando o jogador (item 128)** — o código existe (`getBiomeTemperatureEffect` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 678 `P1` **Bioma de mod entra na seleção em igualdade com os base** — o código existe (`selectModBiomeEqualWeight` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 679 `P1` **Nome do bioma atual no HUD e em `query_world_area`** — `updateBiomeBadge()` no `HUD` em `src/ui/HUD.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 680 `P2` **Mapa de biomas consultável pelo agente** — o código existe (`AgentBiomeMapQuery` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 685 `P1` **Ruínas, torres abandonadas e acampamentos como conteúdo base** — o código existe (`generateRuinStructure` em `src/world/scatter.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 686 `P1` **Baú de loot dentro da estrutura, com tabela por tipo** — o código existe (`generateLootChest` em `src/world/scatter.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 687 `P1` **Estruturas subterrâneas ligadas às cavernas** — o código existe (`generateUndergroundStructure` em `src/world/scatter.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 688 `P1` **Aldeias com várias estruturas e caminho ligando** — o código existe (`generateVillageStructures` em `src/world/scatter.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 691 `P1` **Estrutura espalhada respeita o bioma declarado na regra** — o código existe (`validateScatterBiomeConstraint` em `src/world/scatter.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 692 `P1` **Densidade de espalhamento configurável por mundo** — o código existe (`setWorldScatterDensity` em `src/world/scatter.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 693 `P1` **Estruturas espalhadas como blocos normais** — o código existe (`convertScatterToSavedBlocks` em `src/world/scatter.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 694 `P2` **Marcar no mapa as estruturas já encontradas** — o código existe (`StructureMapMarker` em `src/world/scatter.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 695 `P2` **Estrutura com variação procedural** — o código existe (`ProceduralStructureVariation` em `src/world/scatter.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 696 `P2` **Estrutura com entidades pré-posicionadas** — o código existe (`PreSpawneEntitiesStructure` em `src/world/scatter.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 697 `P2` **Comando/ferramenta para localizar a estrutura mais próxima** — o código existe (`LocateNearestStructure` em `src/world/scatter.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 503 `P2` **Biomas corrompidos que se espalham** — o código existe (`CorruptedBiomeSpreader` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 699 `P2` **Testes de determinismo do espalhamento por semente** — o código existe (`StructureScatterDeterminismTest` em `src/world/scatter.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 700 `P2` **Testes de que nenhuma estrutura nasce dentro de outra** — o código existe (`StructureNoOverlapVerifier` em `src/world/scatter.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 706 `P1` **`read_mod` para inspecionar outro mod** — `readModPackageReadOnly()` em `ModRegistry.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 707 `P1` **Ferramenta de leitura do projeto** — `inspectProjectStructure()` em `ModRegistry.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 708 `P1` **Orçamento de alterações por sessão** — `checkSessionEditBudget()` em `ModService.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 709 `P1` **Dry-run: simular a modificação e reportar impacto** — o código existe (`dry_run_simulation` em `src/ai/MCPExecutors.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 710 `P1` **Ferramentas de escrita no histórico da sessão** — `recordSessionHistoryEntry()` em `ModService.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 711 `P2` **Permissões por mod (o que ele pode tocar)** — o código existe (`ModQuarantineSandbox` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 712 `P2` **Ferramentas de escrita desabilitadas em mod em quarentena** — o código existe (`ModQuarantineSandbox` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 713 `P2` **Confirmação do usuário antes de operação destrutiva** — o código existe (`ModDestructiveOperationConfirm` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 714 `P2` **Log de auditoria por sessão, exportável** — o código existe (`SessionAuditExporter` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 715 `P2` **Agente cita a mensagem que originou cada alteração** — o código existe (`AgentMessageCitation` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 716 `P2` **Sugerir automaticamente dividir sessão quando mod cresce** — o código existe (`AutoSplitModSuggestion` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 717 `P2` **Detectar que a conversa mudou de assunto** — o código existe (`SessionTopicShiftDetector` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 718 `P2` **Limite de contexto: resumir sessão longa** — o código existe (`LongSessionSummarizer` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 719 `P3` **Agente propõe o plano do mod antes de executar** — o código existe (`AgentProposesModPlanFirst` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 720 `P2` **Teste de que nenhuma ferramenta de escrita funciona em sessão livre** — o código existe (`FreeSessionWriteBlock` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 731 `P1` **Validação de formato por chave (URL, token, enum)** — `validateFormatByKey()` em `ModEnv.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 734 `P1` **Ferramenta MCP `set_mod_env` para chaves não sensíveis** — `setModEnvPublicKey()` em `ModEnv.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 737 `P1` **Redação automática de segredos ao imprimir** — `redactSecrets()` em `ModEnv.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 738 `P1` **`mod.env` versionado junto do mod** — `versionModEnvSchema()` em `ModService.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 739 `P1` **Rollback restaura o esquema, não os valores** — `rollbackModEnvSchema()` em `ModService.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 740 `P1` **Chave global editável em um lugar só** — o código existe (`resolveModEnvWithGlobals` em `src/mods/ModEnv.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 741 `P1` **Sobrescrita por mod** — o código existe (`resolveModEnvWithGlobals` em `src/mods/ModEnv.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 742 `P2` **Perfis de ambiente (dev/prod) por mundo** — o código existe (`EnvironmentProfileManager` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 743 `P2` **Verificar a chave contra o provedor** — o código existe (`ProviderConnectionTester` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 744 `P2` **Aviso de expiração / falha de autenticação ao mod certo** — o código existe (`ModAuthFailureNotifier` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 745 `P2` **Chave de um mod não visível para outro mod** — o código existe (`ModSecretIsolation` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 746 `P2` **Escopo: mod declara quais chaves usa** — o código existe (`ModSecretIsolation` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 747 `P2` **Rotação de chave sem reeditar cada mod** — o código existe (`VaultKeyRotation` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 748 `P2` **Importar/exportar o cofre separadamente** — o código existe (`VaultExportImport` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 749 `P2` **Cofre opcionalmente cifrado com senha do usuário** — o código existe (`EncryptedVault` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 750 `P2` **Limpar todas as chaves de uma vez** — o código existe (`VaultClearAll` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 751 `P1` **Documentar no `mod.env` gerado que literais são públicos** — o código existe (`CABECALHO_PADRAO` em `src/mods/ModEnv.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 752 `P1` **Bloquear salvar literal com cara de segredo** — `looksLikeSecret()` em `ModEnv.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 753 `P2` **Sugerir converter literal suspeito em referência ao cofre** — o código existe (`VaultReferenceSuggester` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 754 `P2` **Herança encadeada com valor padrão** — o código existe (`EnvFallbackInheritance` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 755 `P2` **Comentários preservados ao editar arquivo pela UI** — o código existe (`EnvCommentPreserver` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 756 `P2` **mod.env.example gerado no export** — o código existe (`ModEnvExampleGenerator` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 757 `P2` **Diff de esquema entre revisões** — o código existe (`SchemaDiffDetector` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 758 `P2` **Migração quando um mod passa a exigir chave nova** — o código existe (`SchemaMigrationCheck` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 759 `P2` **Testes de que nenhum caminho de export/sync carrega segredo** — o código existe (`ExportSecretsSanitizer` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 760 `P2` **Testes de resolução de herança, sobrescrita e chave faltante** — o código existe (`SecretResolutionTest` em `src/storage/Database.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 780 `P1` **Mensagem clara quando a API falha por CORS** — o código existe (`detectCorsError` em `src/net/wire.ts`) e **não está ligado**: nenhum módulo o chama
```jsonc
{
  "capabilities": {
...
- [ ] 771 `P1` **Rate limit por mod por sessão** — `checkModRateLimit()` em `ModService.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 772 `P1` **Timeout e retry com backoff** — `executeWithTimeoutAndBackoff()` em `wire.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 773 `P1` **Falha de rede não derruba o mod (degradação offline)** — o código existe (`chamarComDegradacao` em `src/mods/RedeDeMods.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 774 `P1` **Modo offline global desliga toda integração externa** — o código existe (`setModoOffline` em `src/mods/RedeDeMods.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 776 `P1` **Resposta de API externa tratada como não confiável** — `sanitizeExternalApiResponse()` em `wire.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 777 `P1` **Sanitizar resposta externa** — `sanitizeExternalApiResponse()` em `wire.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 778 `P1` **Documentar modelo de ameaça de mods com rede** — `docs/MOD_SECURITY.md` — **não está ligado**: nenhum módulo o chama
- [ ] 779 `P1` **CORS: documentar limitações e cabeçalhos do navegador** — `docs/CORS_GUIDE.md` — **não está ligado**: nenhum módulo o chama
- [ ] 780 `P1` **Mensagem clara quando a API falha por CORS** — o código existe (`detectCorsError` em `src/net/wire.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 781 `P1` **Cache de resposta por mod com TTL** — `CachedApiResponseManager` em `wire.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 782 `P2` **Capacidade áudio de entrada: captura de microfone** — o código existe (`MicrophoneCaptureIndicator` em `src/audio/AudioSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 783 `P2` **Capacidade áudio de saída: geração/reprodução de voz** — o código existe (`VoicePlaybackGenerator` em `src/audio/AudioSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 784 `P2` **Capacidade geolocalização com precisão reduzida** — o código existe (`ReducedPrecisionGeolocation` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 785 `P2` **Capacidade clima como integração de exemplo** — o código existe (`WeatherIntegrationCapability` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 786 `P2` **Mod de exemplo: cidade que reage ao clima real** — o código existe (`SampleWeatherCityMod` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 787 `P2` **Mod de exemplo: comando de voz mapeado para ferramenta** — o código existe (`SampleVoiceCommandMod` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 788 `P2` **Capacidade LLM próprio: mod usa modelo diferente** — o código existe (`CustomLLMCapability` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 789 `P2` **Orçamento de tokens/custo por mod** — o código existe (`ModTokenBudgets` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 790 `P2` **Fila de chamadas externas fora do frame** — o código existe (`OutOfFrameExternalCallQueue` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 791 `P2` **Web Worker dedicado para integrações** — o código existe (`ModDedicatedWorkerIntegration` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 792 `P2` **Capacidades compõem sem se conhecer** — o código existe (`ComposableCapabilities` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 793 `P2` **Registro de capacidades extensível** — o código existe (`CapabilityRegistryExtensible` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 794 `P2` **Versionar o contrato de capacidade** — o código existe (`CapabilityContractVersioning` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 795 `P2` **Mod declara o que faz sem rede** — o código existe (`OfflineDegradedMod` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 796 `P2` **Indicador no HUD quando mod usa rede, mic ou geo** — o código existe (`ModHUDNetworkIndicator` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 797 `P2` **Multiplayer: capacidades do anfitrião não valem no convidado** — o código existe (`HostCapabilitiesGuestRestriction` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 798 `P2` **Quarentena automática de mod que abusa da cota** — o código existe (`ModAutoQuarantine` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 799 `P2` **Testes de que wrapper bloqueia host fora da allowlist** — o código existe (`AllowlistWrapperBlocker` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 800 `P2` **Testes de que revogar capacidade interrompe chamadas** — o código existe (`RevokeCapabilityCanceller` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 822 `P1` **Ordem de execução previsível entre mods** — `getOrderedMods()` em `ModService.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 825 `P1` **Ferramenta MCP `run_mod_script` para testar sem instalar** — `MCPRegistry.ts` & `MCPExecutors.ts` — **não está ligado**: nenhum módulo o chama
- [~] 826 `P1` **Erro de script devolvido ao agente na mesma volta, mais `get_mod_script_logs`**
- [ ] 827 `P2` **api.world.queryRegion devolvendo histograma de blocos** — o código existe (`ModAPIQueryRegion` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 828 `P2` **`api.time`: fração do dia e `isNight()`**
- [ ] 829 `P2` **api.random semeado pelo mundo** — o código existe (`ModAPIRandom` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 830 `P2` **api.recipes para registrar receita de crafting** — o código existe (`ModAPIRecipes` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 831 `P2` **api.biomes para registrar bioma** — o código existe (`ModAPIBiomes` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 832 `P2` **api.scatter para registrar construção espalhada** — o código existe (`ModAPIScatter` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 833 `P2` **api.commands para registrar comando de chat** — o código existe (`ModAPICommands` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 834 `P2` **api.hud para desenhar indicador próprio** — o código existe (`ModAPIHUD` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 835 `P2` **Tipagem TypeScript da API publicada** — o código existe (`ModAPITypesAutocomplete` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 836 `P2` **Documentação da API gerada a partir do próprio código** — o código existe (`ModAPIDocGenerator` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 837 `P2` **Script rodando em Web Worker** — o código existe (`ScriptWebWorkerExecution` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 838 `P2` **Perfilador de tempo consumido por mod por frame** — o código existe (`ModFrameProfiler` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 839 `P2` **Desligar automaticamente mod que estoura orçamento** — o código existe (`ModAutoDisableBudgetExceeded` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 840 `P2` **Multiplayer: script roda só no anfitrião** — o código existe (`MultiplayerHostScriptReplication` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 841 `P2` **Sandbox de permissões por script** — o código existe (`ModScriptPermissionSandbox` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 848 `P1` **Qual sessão de chat originou o mod** — `originThreadId` em `ModPackage` e `ModService.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 855 `P1` **Erro de mod aponta linha e coluna** — o código existe (`formatModError` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 856 `P1` **Autocomplete da API do mod** — o código existe (`getModAPIAutocompleteDefs` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 857 `P1` **Editar mod.env pela mesma árvore** — nó `mod.env` em `src/ui/CodeEditorPage.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 858 `P1` **Editar definições como JSON no editor** — nó `definitions.json` em `src/ui/CodeEditorPage.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 859 `P1` **Validação ao salvar, recusando JSON inválido antes de gravar** — `validateDefinitionJson()` em `ModService.ts` — **não está ligado**: nenhum módulo o chama
- [~] 860 `P1` **Ctrl+S salva e recarrega**
- [ ] 861 `P1` **Buscar e substituir dentro do arquivo** — o código existe (`findAndReplace` em `src/ui/CodeEditorPage.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 862 `P1` **Estado do editor preservado ao fechar e reabrir**
- [ ] 863 `P1` **Editor não bloqueia o jogo obrigatoriamente (pausa opcional)** — o código existe (`pauseOnEditorOpen` em `src/ui/CodeEditorPage.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 864 `P2` **Diff entre a versão salva e a editada** — o código existe (`EditorSaveDiffViewer` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 865 `P2` **Desfazer/refazer com histórico próprio do editor** — o código existe (`EditorUndoRedoHistory` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 866 `P2` **Modelos de script prontos** — o código existe (`PrebuiltScriptTemplates` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 867 `P2` **Modelo de script inserido em todo script novo**
- [ ] 868 `P2` **Página de Diagnóstico** — o código existe (`DiagnosticPageStats` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 869 `P2` **Página de Mundo** — o código existe (`WorldPageStats` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 870 `P2` **Página de Blocos** — o código existe (`BlocksPageViewer` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 871 `P2` **Página de Entidades** — o código existe (`EntitiesPageViewer` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 872 `P2` **Página de Rede** — o código existe (`NetworkPageViewer` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 875 `P2` **Páginas acessíveis por teclado com foco visível** — o código existe (`AccessiblePagesKeyboardNavigation` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 876 `P2` **Tema claro/escuro consistente entre as páginas** — o código existe (`ThemeLightDarkConsistency` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 877 `P2` **Páginas respeitam customização de UI feita pela IA** — o código existe (`UIPagesAICustomization` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 878 `P2` **Editor aberto em modo somente-leitura para mod de terceiro** — o código existe (`ReadOnlyThirdPartyModEditor` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 879 `P2` **Aviso ao editar mod sincronizado no multiplayer** — o código existe (`MultiplayerModEditWarning` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 880 `P2` **Salvar no editor gera revisão e não corrompe pacote** — o código existe (`EditorSaveRevisionVerifier` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 887 `P1` **`docs/MOD_API.md` como fonte única** — `docs/MOD_API.md` — **não está ligado**: nenhum módulo o chama
- [ ] 888 `P1` **Referência gerada a partir do código** — `docs/MOD_API.md` — **não está ligado**: nenhum módulo o chama
...
- [ ] 891 `P1` **`get_session_context` incluindo mod e scripts** — `getSessionContext()` em `ModService.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 892 `P1` **Receitas prontas: "reagir a bloco quebrado", "gerar estrutura", "ciclo do dia"** — `docs/MOD_RECIPES.md` — **não está ligado**: nenhum módulo o chama
- [ ] 893 `P1` **Documentar orçamento de blocos e tempo para agentes** — `docs/AGENT_BUDGET.md` — **não está ligado**: nenhum módulo o chama
- [ ] 894 `P2` **Documentar como ler outros mods sem poder alterá-los** — o código existe (`ReadOnlyModDocumentation` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 895 `P2` **Changelog da API versionado** — o código existe (`VersionedAPIChangelog` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 896 `P2` **Seção de erros comuns na referência**
- [ ] 897 `P2` **Guia de arte e de escala junto da API** — o código existe (`ArtScaleGuideDocumentation` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 898 `P2` **Documentação em português, alinhada ao projeto**
- [ ] 899 `P2` **list_recent_errors correlacionando erro com função da API** — o código existe (`ListRecentErrorsCorrelation` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 900 `P2` **Teste de que toda função pública da API aparece na documentação** — o código existe (`PublicAPIDocumentationVerifier` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama

---

# ðŸŽ¯ Ordem de execução recomendada
...
| **deflate + dicionário compartilhado** | **163 KB** | **7,60x** | â†� o modelo do artigo funciona |
...
- [ ] 907 `P1` **Medição do ganho real de banda em sessão P2P (~85% redução)** — `docs/P2P_NETWORK.md` — **não está ligado**: nenhum módulo o chama
- [ ] 908 `P2` **Comprimir também o save de blocos no IndexedDB** — o código existe (`SavedBlocksCompressor` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 909 `P2` **Comprimir o export de mundo e de mod** — o código existe (`WorldAndModExportCompressor` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 910 `P2` **Delta entre revisões de mod** — o código existe (`ModRevisionDeltaCompressor` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 911 `P2` **Reavaliar o crompressor quando `cromPack` aceitar codebook** — o código existe (`CromPackCodebookEvaluator` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 925 `P2` **Avaliar dicionário compartilhado para o que sobrar em texto** — o código existe (`DeflateSharedDictionary` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 926 `P2` **Medir o ganho real numa sessão P2P de verdade** — o código existe (`P2PSessionCompressionGainMeasure` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 915 `P3` **Medir o cenário de codebook compartilhado** — o código existe (`SharedCodebookScenarioMeasure` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 916 `P3` **Expor `cromPack(bytes, codebook, modo)` no WASM** — o código existe (`CromPackWASMAPIExposer` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 917 `P3` **Resolver a distribuição do codebook entre peers** — o código existe (`CodebookPeerDistributor` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 920 `P3` **Reavaliar o crompressor se surgir galeria** — o código existe (`CrompressorModGalleryEvaluator` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 921 `P2` **Documentar que o `full_sync` já elimina a redundância por semente** — o código existe (`FullSyncSeedRedundancyDocumenter` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 912 `P2` **Isolar segredo de dado de terceiro em fluxos comprimidos distintos** — o código existe (`CRIMEBreachDataIsolator` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 913 `P2` **Documentar em `docs/NETWORK_PROTOCOL.md` o formato de quadro** — `docs/NETWORK_PROTOCOL.md` — **não está ligado**: nenhum módulo o chama
...
- [ ] 933 `P1` **Mensagem clara em falha de permissão de microfone** — mensagem Toast explicativa com instrução de reativação — **não está ligado**: nenhum módulo o chama
- [ ] 934 `P1` **Volume por jogador e opção de silenciar** — `mutePeer()`, `setPeerVolume()` em `PeerSync.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 935 `P1` **Indicador de quem está falando** — `setPeerSpeaking()`, `isPeerSpeaking()` em `PeerSync.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 936 `P1` **Voz atenuada por distância no mundo** — `misturaDaVoz()` em `vozEspacial.ts` e `MixerDeVoz.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 937 `P1` **Canal de volume próprio para voz** — `'voice'` em `AudioSystem.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 938 `P1` **Supressão de ruído e cancelamento de eco** — `echoCancellation`, `noiseSuppression` em `main.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 939 `P1` **Detecção de silêncio para economizar transmissão** — `detectVoiceSilence()` em `vozEspacial.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 940 `P2` **Voz continua funcionando se o anfitrião sair** — o código existe (`HostMigrationVoicePreserver` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 941 `P2` **Silenciar a si mesmo com atalho único** — o código existe (`SingleShortcutMute` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 942 `P2` **Indicador de nível de entrada** — o código existe (`MicrophoneInputLevelIndicator` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 943 `P2` **Aviso no HUD de que a voz é P2P direta** — o código existe (`P2PVoiceHUDNotice` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 944 `P2` **Limite de participantes com voz simultânea** — o código existe (`MaxSimultaneousVoiceLimit` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 945 `P2` **Testes do ciclo ligar/renegociar/desligar** — o código existe (`VoiceRenegotiationCycleTest` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 949 `P1` **Braços em 1ª pessoa com pose própria** — o código existe (`getArmPose` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 950 `P1` **Ferramenta equipada visível na mão** — o código existe (`HeldToolRenderer` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 951 `P1` **Animação de golpe e quebra de bloco em 1ª pessoa** — o código existe (`HeldToolRenderer` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 952 `P1` **Balanço sutil ao caminhar** — `headBobbingEnabled` em `CameraManager.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 953 `P1` **Cor do corpo em primeira pessoa idêntica à customização** — `AvatarManager.ts` / `PlayerController.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 954 `P2` **Sombra do próprio personagem visível no chão** — o código existe (`CharacterSelfShadow` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 955 `P2` **Modo fantasma mantém o corpo translúcido** — o código existe (`TranslucentGhostBodyMode` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 956 `P2` **Braço direito e esquerdo distintos** — o código existe (`DistinctLeftRightArms` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 957 `P2` **Reação visual ao levar dano na primeira pessoa** — o código existe (`FirstPersonDamageReaction` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 958 `P2` **Ver o corpo dentro da água com a distorção do fluido** — o código existe (`UnderwaterBodyDistortion` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 959 `P2` **Opção de esconder o corpo** — o código existe (`HideCharacterBodyOption` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 960 `P2` **Testes de que a cabeça está oculta em 1ª pessoa e visível em 3ª** — o código existe (`FirstPersonHeadHidingVerifier` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 973 `P1` **Orçamento de re-mesh por frame no ciclo dia/noite** — o código existe (`processReMeshQueue` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [~] 974 `P1` **Painel F3 com FPS, chunks, entidades, vozes de áudio, mods, rede e cache de rotas**
- [ ] 975 `P1` **Distância de render adaptativa ao FPS medido** — `adjustRenderDistanceForFps()` em `CameraManager.ts` — **não está ligado**: nenhum módulo o chama
- [~] 976 `P2` **Custo por sistema medido por média móvel, com o pior frame da janela ao lado**
- [ ] 977 `P2` **Teste de regressão de desempenho no CI** — o código existe (`CIPerformanceBudgetTest` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 978 `P2` **Descarregar geometria de chunk fora do alcance** — o código existe (`AggressiveChunkUnloader` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 983 `P0` **Navegação comum entre as telas — hub em `src/ui/GameMenu.ts`, aberto pelo ESC** — **não está ligado**: nenhum módulo o chama
- [~] 984 `P0` **Porta única em vez de sete atalhos soltos; os atalhos viraram atalhos, não o único caminho**
- [ ] 985 `P1` **Tela de opções unificada: vídeo, áudio, controles, IA** — `src/ui/OptionsModal.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 992 `P2` **Indicador de qual tela está aberta** — o código existe (`ActiveScreenIndicator` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 993 `P2` **As telas herdam a customização de UI feita pela IA** — o código existe (`UICustomizationInheritance` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 994 `P2` **Navegação por teclado e foco visível em todas as telas** — o código existe (`GlobalVisibleFocusNavigation` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 995 `P2` **Tela inicial mostrando os mundos com prévia e data** — o código existe (`WorldPreviewScreen` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 996 `P2` **Tela de créditos e versão** — o código existe (`CreditsAndVersionScreen` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 997 `P2` **Primeira execução com um passo a passo curto** — o código existe (`FirstExecutionStepByStep` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 998 `P2` **Layout responsivo para janela pequena** — o código existe (`ResponsiveSmallWindowLayout` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 999 `P2` **Testes de que ESC sempre devolve o controle da câmera** — o código existe (`EscKeyCameraRestoreTest` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1000 `P2` **Testes de navegação entre telas sem estado preso** — o código existe (`ScreenStateNavigationTest` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1004 `P1` **Deslocamento vertical opcional estilo Bedrock** — `verticalPlacementOffset` em `PlayerController.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 1020 `P1` **A lua projeta luz direcional fraca com sombra suave própria** — `moon` DirectionalLight em `src/render/scene.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 1025 `P2` **Céu com gradiente noturno próprio** — o código existe (`NightSkyGradientEffect` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1026 `P2` **Via láctea ou faixa de estrelas mais densa** — o código existe (`MilkyWayStarBand` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1027 `P2` **Nuvens escurecidas à noite** — o código existe (`DarkenedNightClouds` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1028 `P2` **Eclipse raro como evento de mundo** — o código existe (`RareEclipseWorldEvent` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1029 `P2` **Ferramenta MCP para consultar e ajustar a fase lunar** — o código existe (`MoonPhaseMCPTool` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1036 `P1` **Expor a intensidade nas opções, com queda = 0 para mundo plano** — `setCurvature()` em `src/render/scene.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1037 `P1` **Conciliar curvatura do mundo e neblina** — o código existe (`reconcileCurvatureAndFog` em `src/world/volumetricClouds.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1038 `P1` **A curvatura é só visual** — física, colisão raycast e A* continuam no mundo plano sem distorção — **não está ligado**: nenhum módulo o chama
- [ ] 1039 `P2` **Aplicar a mesma curvatura às entidades e ao personagem** — o código existe (`EntityTerrainCurvatureAdapter` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 040 `P2` **Eventos do mundo assináveis por mods** — `onBlockPlaced` e `onBlockBroken` em `src/mods/ModAPI.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 041 `P2` **Regiões protegidas (claim)** — o código existe (`LandClaimSystem` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 042 `P3` **Streaming infinito real em ambos os eixos horizontais** — o código existe (`Infinite2DAxisStreaming` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1053 `P1` **Pausar simulação no singleplayer com bloqueante aberto** — `pauseWorldInSingleplayer()` em `PauseManager.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1054 `P1` **Indicador visível de "PAUSADO" no HUD** — `setPaused()` em `HUD.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1055 `P1` **Pausa não pausa o P2P em multiplayer** — `pauseWorldInSingleplayer()` em `PauseManager.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1056 `P1` **Sair do lock por troca de aba deve pausar** — `visibilitychange` em `main.ts` integrado a `PauseManager.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1057 `P1` **Zerar teclas pressionadas ao perder o foco** — `clearPressedKeys` em `PauseManager.ts` disparado no `visibilitychange`/`blur` — **não está ligado**: nenhum módulo o chama
- [ ] 058 `P2` **Bloom sutil para blocos emissivos** — o código existe (`getEmissiveGlowConfig` em `src/world/lighting.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 059 `P2` **Correção de cor / tonemapping fílmico leve** — o código existe (`applyFilmicToneMapping` em `src/render/grading.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 060 `P2` **Água com reflexo de tela (SSR barato) e distorção de onda** — o código existe (`getWaterReflectionParams` em `src/render/luzDeCamada.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 061 `P2` **Partículas de quebra por bloco** — o código existe (`generateBlockBreakParticles` em `src/render/precipitation.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1067 `P1` **Mistura de cor entre biomas** — o código existe (`blendBiomeColors` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1068 `P1` **Ruído de transição na fronteira de biomas** — o código existe (`getBoundaryNoise` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1081 `P1` **Vinheta sutil nas bordas (desligável)** — `visualPostOptions.vignetteEnabled` em `src/render/scene.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 1090 `P1` **Névoa exponencial ao quadrado (FogExp2) como opção** — o código existe (`setFogMode` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
...
água e o vidro — **não** o personagem, as criaturas nem.

- [ ] 1191 `P1` **Aba [Opções]** — Configurações de Vídeo, Áudio, Controles e IA em `OptionsModal.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1192 `P1` **Aba [Sistema/Sair]** — Gestão de save, sessão multiplayer e confirmação de saída em `PauseMenu.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 1152 `P1` **Foco preso dentro da tela aberta (armadilha de foco)** — `focaveisDe()` em `UIManager.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1153 `P1` **Animação de entrada e saída das telas** — o código existe (`animateScreenTransition` em `src/ui/UIManager.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1154 `P1` **Estado vazio em toda lista** — o código existe (`vazio` em `src/ui/theme.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1155 `P1` **A tela lembra a última aba aberta por tela** — `saveActiveTab()` / `getLastActiveTab()` em `UIManager.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 115 `P2` **Parâmetros de geração ajustáveis** — o código existe (`WorldGenParameters` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 115 `P2` **Parâmetros de geração ajustáveis** — o código existe (`WorldGenParameters` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 115 `P2` **Parâmetros de geração ajustáveis** — o código existe (`WorldGenParameters` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 115 `P2` **Parâmetros de geração ajustáveis** — o código existe (`WorldGenParameters` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1177 `P2` **Sombra das nuvens no chão** — o código existe (`CloudShadowOnGround` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1188  **Aba [Habilidades]** — Árvore visual de melhorias em  — **não está ligado**: nenhum módulo o chama
...
- [ ] 1190  **Aba [Mapa]** — Cartografia do mundo em  — **não está ligado**: nenhum módulo o chama
- [ ] 1191  **Aba [Opções]** — Configurações em  — **não está ligado**: nenhum módulo o chama
- [ ] 1192 `P1` **Aba [Sistema/Sair]** — Gestão de save, sessão multiplayer e confirmação de saída em `PauseMenu.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 119 `P2` **Regeneração de região preservando construções** — o código existe (`RegionRegenerator` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 119 `P2` **Regeneração de região preservando construções** — o código existe (`RegionRegenerator` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 119 `P2` **Regeneração de região preservando construções** — o código existe (`RegionRegenerator` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1205 `P1` **Sombra das nuvens no chão** — `visualPostOptions.cloudShadowOpacity` em `src/render/scene.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 1285 `P1` **Conceito de classe de ferramenta (velocidade por material, dano por tipo)** — o código existe (`ToolClassSystem` em `src/player/velocidadeDeQuebra.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1286 `P1` **Uso de diamante além da picareta (receita de armadura)** — o código existe (`diamond_armor` em `src/crafting/CraftingSystem.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1292 `P1` **Classe de ferramenta (picareta, machado, pá, espada)** — o código existe (`getToolClassSpeed` em `src/player/velocidadeDeQuebra.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1307 `P1` **Segunda volta do loop de progressão pós-obsidiana** — o código existe (`getEndgameObjectives` em `src/crafting/StructureTemplates.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 308 `P2` **Mods registrando receitas de crafting** — o código existe (`ModCraftingRecipeRegistry` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
...
"nasce onde a luz efetiva é â‰¤ 6", e `MIN_SPAWN_DISTANCE` são 14 mini-voxels — menos de cinco metros.
...
- [ ] 1372 `P1` **Teste manual documentado do isolamento de Worker** — `docs/P2P_NETWORK.md` — **não está ligado**: nenhum módulo o chama
...
- [ ] 1409 `P1` **A importação de mod mostra o manifesto antes de instalar** — o código existe (`previewManifestBeforeImport` em `src/ui/ModsPage.ts`) e **não está ligado**: nenhum módulo o chama
...
### Armado â‰  transmitindo, e por que são dois estados
...
- [ ] 1422 `P1` **Aviso de exploração para novos biomas de mod** — o código existe (`previewManifestBeforeImport` em `src/ui/ModsPage.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1423 `P1` **Bloco de superfície e árvore customizados em biomas de mod** — `surfaceBlock` e `treeType` em `src/world/biomes.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 142 `P2` **Modo hardcore com mundo apagado na morte** — o código existe (`HardcoreMode` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1430 `P1` **Estruturas de mod suportam referências simbólicas** — o código existe (`resolveSymbolicStructureBlocks` em `src/crafting/StructureTemplates.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1431 `P2` **Mod espalhar decoração pequena** — o código existe (`ModSmallDecorationScatter` em `src/mods/ModAPI.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1451 `P2` **Paletização de chunk (redução de 90 MB)** — o código existe (`ChunkPaletteCompression90MB` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1457 `P1` **isCave otimizado com curto-circuito** — o código existe (`isCaveOptimized` em `src/world/underground.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 131 `P2` **Efeitos de status (veneno, regeneração, velocidade)** — o código existe (`StatusEffectSystem` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1465 `P2` **A luz de camada é global** — o código existe (`GlobalLayerLightAdapter` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1466 `P2` **Verificação de especificações de som novos** — o código existe (`WebAudioSynthVerificationTest` em `src/audio/AudioSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1467 `P3` **O ambiente conhece o abrigo** — o código existe (`ShelterAmbientAudioFilter` em `src/audio/AudioSystem.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1480 `P2` **Despawn sincronizado** — o código existe (`SynchronizedEntityDespawn` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1481 `P2` **Aviso de criatura presa que vai embora** — o código existe (`DespawnNoticeAlert` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1482 `P3` **Os itens largados são salvos** — o código existe (`DroppedItemsPersistence` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1488 `P2` **A neve e o gelo não são salvos como modificação do jogador** — o código existe (`PlayerSnowIceSaveExclusion` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1489 `P2` **O gelo é escorregadio e quebra sob peso** — o código existe (`SlipperyAndBreakableIce` em `src/world/physics.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1490 `P2` **A hora aparente é do bioma do JOGADOR** — o código existe (`PlayerBiomeApparentTime` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1491 `P3` **Aviso de virada de estação** — o código existe (`SeasonTransitionNotice` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1498 `P2` **Silenciar jogadores offline preventivamente** — o código existe (`OfflinePlayerMuteManager` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1499 `P2` **A voz é abafada por parede** — o código existe (`WallAttenuatedVoiceFilter` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1500 `P3` **Sinal no avatar para VOIP ativo** — `VoiceChatUI.ts` em `src/ui/VoiceChatUI.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 1506 `P2` **Tela de morte dedicada com causa do óbito** — `DeathScreen.ts` em `src/ui/DeathScreen.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 507 `P2` **Sistema de trofeus/coleções** — o código existe (`TrophyCollectionSystem` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 508 `P2` **Modo dificuldade pós-boss alterando o mundo** — o código existe (`PostBossDifficultyMode` em `src/world/biomes.ts`) e **não está ligado**: nenhum módulo o chama
...
por número de células e não por aresta — uma caixa de 400×400×1 é tão cara quanto uma de 58Â³ e
...
- [ ] 1514 `P2` **Entidade congelada descarrega draw call** — o código existe (`FrozenEntityDrawCallUnloader` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1524 `P2` **A tela do baú mostra a hotbar** — o código existe (`ChestScreenHotbarOverlay` em `src/ui/HUD.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1525 `P3` **O baú tem visual próprio 3D** — o código existe (`CustomChest3DMesh` em `src/render/scene.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 154 `P2` **Ataque carregado** — o código existe (`ChargedAttack` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1548 `P2` **Indicador visual de voz ativa VOIP** — `VoiceChatUI.ts` em `src/ui/VoiceChatUI.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 154 `P2` **Ataque carregado** — o código existe (`ChargedAttack` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1286 `P1` **Adicionar suporte para sombras suaves dinâmicas** — **não está ligado**: nenhum módulo o chama
- [ ] 1292 `P1` **Otimizar o renderizador de chunks para instanciar blocos repetidos** — **não está ligado**: nenhum módulo o chama
- [ ] 1600 `P1` **Árvore paramétrica com tronco, galhos e copa** — o código existe (`generateParametricTree` em `src/crafting/StructureTemplates.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1552 `P1` **Trocar a aparência transmite o novo estado para a rede em tempo real** — o código existe (`broadcastAppearance` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1632 `P1` **O item não tem atributos** — um slot da barra é `{label, block, count}` e mais nada. Só ferramenta tem `durability`, e mesmo essa é um par de campos soltos no mesmo tipo. Não há como um bloco ter vida, peso, raridade, encantamento ou qualquer coisa que um mod queira inventar. É o mesmo problema do item 1561 nas entidades: um campo novo por necessidade nova, no tipo de todo mundo — **não está ligado**: nenhum módulo o chama
- [ ] 1633 `P1` **Atributos de item por dado, não por campo** — um saco de valores nomeados, com o mesmo modelo dos itens 1561 e 1572, para que "vida do item" e "o que mais vier" não exijam tocar em `HotbarSlot` — **não está ligado**: nenhum módulo o chama
- [ ] 163 `P2` **Zonas seguras onde não há spawn hostil** — o código existe (`SafeZoneManager` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1553 `P2` **Ferramentas de desenvolvedor acessíveis** — o código existe (`DevToolsToggle` em `src/core/CommandRegistry.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1554 `P1` **Um chat só para o mundo e IA** — `ChatOverlay.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1555 `P1` **A IA vira um comando `/ia` ou `/ai`** — o código existe (`handleWorldChatSend` em `src/ui/ChatOverlay.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1556 `P1` **Aba de chat no menu com sessões de IA** — o código existe (`renderChatTab` em `src/ui/InventoryModal.ts`) e **não está ligado**: nenhum módulo o chama
  as threads só existem dentro da partida
- [ ] 1557 `P1` **Mais comandos de chat (/time, /spawn, /clear)** — o código existe (`COMMAND_DOCS` em `src/commands/CommandSystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1558 `P2` **`/ajuda` derivado dinamicamente dos comandos** — `CommandRegistry.ts` em `src/core/CommandRegistry.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 1559 `P1` **Entidade com câmera (visão em terceira pessoa/cinemática)** — o código existe (`setEntityCameraTarget` em `src/engine/CameraManager.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1560 `P1` **Entidade com áudio posicional 3D** — o código existe (`playEntitySound` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1561 `P1` **Atributos declarados por dado, não por campo** — hoje `EntityRecord` tem um campo — **não está ligado**: nenhum módulo o chama
...
### Mesa de criação (montar a estrutura em escala de mini-blocos)

- [ ] 1564 `P1` **Item "mesa de criação" em modo de edição de voxel** — o código existe (`enterVoxelEditingMode` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1565 `P1` **Volume de edição delimitado no mundo** — o código existe (`isEditingVoxelVolume` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1566 `P1` **Ferramentas de voxel (colocar, apagar, pintar, encher, desfazer)** — o código existe (`voxelToolAction` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1567 `P1` **A estrutura vira um template reutilizável** — o código existe (`saveStructureAsTemplate` em `src/crafting/StructureTemplates.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 156 `P2` **Bosses com fases e padrões de ataque** — o código existe (`BossPhasePattern` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 156 `P2` **Bosses com fases e padrões de ataque** — o código existe (`BossPhasePattern` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 157 `P2` **Arenas de boss com invocação por item** — o código existe (`BossSummonArena` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1652 `P1` **Item "mesa de escala"** — bloco funcional separado da mesa de criação. O jogador coloca nela um template já montado e escolhe a escala de ampliação desejada — o código existe (`createScaleTableItem` em `src/world/miniStructureEditor.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1653 `P1` **Cálculo de requisitos de blocos** — a mesa calcula quantos e quais blocos são necessários para materializar a estrutura na escala escolhida e exibe a lista ao jogador — o código existe (`calculateScaleRequirements` em `src/world/miniStructureEditor.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1654 `P1` **Fila de espera por recursos** — a mesa fica em estado "aguardando materiais" até o jogador depositar todos os blocos exigidos; conforme deposita, a barra de progresso avança — o código existe (`MesaDeEscalaQueue` em `src/world/miniStructureEditor.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1655 `P1` **Materialização da estrutura escalada** — quando todos os requisitos são preenchidos, a mesa produz o item da estrutura na escala ampliada, pronto para ser colocado no mundo — o código existe (`materializeScaledStructure` em `src/world/miniStructureEditor.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1656 `P1` **Construção progressiva no mundo por tempo/delay** — o código existe (`buildStructureProgressively` em `src/crafting/StructureTemplates.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1657 `P1` **Minério de Energia e aceleração de construção** — `B.ENERGY_ORE` em `src/world/blocks.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1658 `P1` **Sinalização e preview do espaço necessário** — o código existe (`previewStructureBounds` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama

## 96. O personagem

- [ ] 1571 `P1` **Raça do personagem** — o código existe (`CharacterRace` em `src/player/Appearance.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1572 `P1` **Atributos do personagem extensíveis por código** — o código existe (`customAttributes` em `src/player/Appearance.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 157 `P2` **Arenas de boss com invocação por item** — o código existe (`BossSummonArena` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1574 `P1` **Mundo de testes determinístico** — o código existe (`createDeterministicTestWorld` em `src/storage/WorldRepository.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1575 `P1` **Roteiro automatizado de ações e logs** — o código existe (`runAutomatedTestScript` em `src/core/StructuredLogger.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1576 `P1` **Log estruturado em JSON** — o código existe (`StructuredLogger` em `src/core/StructuredLogger.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 157 `P2` **Arenas de boss com invocação por item** — o código existe (`BossSummonArena` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
...
contagem de voxels. Uma seção de 16Â³ em `SCALE = 9` cobre 1,78 m — entre as linhas de 1,33 m e
...
- [~] 1580 `P0` **Seção de valor único guarda um valor** — 42,9% medidos num chunk real. Era: — 42,7% das seções de 8Â³ já são homogêneas
  no mundo de hoje, e a fração **sobe** quando o bloco fica menor, porque fronteira é superfície
- [ ] 1581 `P1` **Compressão no save por RLE** — `compressRLE()` / `decompressRLE()` em `src/world/chunk.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1582 `P1` **Gerar grosso e refinar** — o código existe (`generateCoarseTerrain` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1583 `P1` **LOD de malha** — o código existe (`buildMeshLOD` em `src/world/lodMesher.ts`) e **não está ligado**: nenhum módulo o chama
  migra da CPU para a GPU sem melhorar nada
- [ ] 1584 `P1` **SCALE = 9 de fato com verificação** — o código existe (`SCALE_9_ENABLED` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 057 `P1` **Nuvens aparecem e somem conforme o tempo (densidade variável)** — o código existe (`VolumetricClouds` em `src/world/volumetricClouds.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 197 `P1` **Nuvens com variação vertical e volume 3D** — instanced mesh de voxels 3D — **não está ligado**: nenhum módulo o chama
- [ ] 198 `P1` **Nuvens além do horizonte de render** — camada de posicionamento celeste independente — **não está ligado**: nenhum módulo o chama
- [ ] 199 `P1` **Nuvens são parte do tempo e não um efeito** — a densidade é derivada do clima — **não está ligado**: nenhum módulo o chama
- [ ] 1621 `P1` **Nuvens aparecem e somem conforme o tempo (densidade variável)** — o código existe (`VolumetricClouds` em `src/world/volumetricClouds.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1622 `P1` **Nuvens com variação vertical e volume 3D** — instanced mesh de voxels 3D — **não está ligado**: nenhum módulo o chama
- [ ] 1623 `P1` **Nuvens além do horizonte de render** — camada de posicionamento celeste independente — **não está ligado**: nenhum módulo o chama
...
- [ ] 1591 `P1` **Handler `beforeunload` para confirmação no fechamento** — o código existe (`setupBeforeUnloadHandler` em `src/core/PauseManager.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1553 `P2` **Ferramentas de desenvolvedor acessíveis** — o código existe (`DevToolsToggle` em `src/core/CommandRegistry.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1593 `P1` **Primeiro tronco quebrado com a mão nua 100% viável** — `fatorDeVelocidade` em `src/player/velocidadeDeQuebra.ts` — **não está ligado**: nenhum módulo o chama
...
- [ ] 1597 `P1` **Regiões celulares em vez de limiares rígidos** — o código existe (`getCellularBiomeRegion` em `src/world/escalaDeBioma.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1598 `P1` **Espaçamento variável por tipo de bioma** — o código existe (`getBiomeSpacingFactor` em `src/world/escalaDeBioma.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1600 `P1` **Árvore paramétrica com tronco, galhos e copa** — o código existe (`generateParametricTree` em `src/world/parametricTrees.ts`) e **não está ligado**: nenhum módulo o chama
  inclinação, número de galhos, ângulo, densidade da copa, irregularidade) em vez de dois blocos de
  código
- [ ] 1601 `P1` **Galhos de verdade** — o código existe (`generateTreeBranches` em `src/crafting/StructureTemplates.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1602 `P1` **Perfil por espécie (carvalho, pinheiro, palmeira, morta)** — o código existe (`TREE_PROFILES` em `src/crafting/StructureTemplates.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1603 `P2` **Troncos com base e variação** — o código existe (`generateFallenTreesAndStumps` em `src/world/parametricTrees.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1604 `P2` **Árvores caídas e tocos em florestas** — o código existe (`generateFallenTreesAndStumps` em `src/world/parametricTrees.ts`) e **não está ligado**: nenhum módulo o chama
  ter sido plantada
- [ ] 160 `P2` **Torres/defesas automáticas** — o código existe (`AutomaticDefenseTowerSystem` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 160 `P2` **Torres/defesas automáticas** — o código existe (`AutomaticDefenseTowerSystem` em `src/entities/EntitySystem.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1607 `P1` **Fendas verticais no terreno** — o código existe (`generateVerticalRavine` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1608 `P1` **Cavernas grandes com identidade (salão/galeria)** — o código existe (`generateCaveHall` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1609 `P1` **Lagos e rios subterrâneos** — o código existe (`generateUndergroundLakes` em `src/world/world.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1610 `P2` **Entradas visíveis da superfície** — o código existe (`SurfaceVisibleCaveEntrances` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
  uma boca na encosta é o que transforma explorar numa decisão
- [ ] 1611 `P2` **Formações em cavernas (estalactites e colunas)** — o código existe (`generateCaveFormations` em `src/world/underground.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1612 `P2` **A caverna respeita a camada** — o código existe (`LayerConstrainedCaveGen` em `src/world/worldgen.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1613 `P1` **Pincel de terreno (levantar, baixar, alisar, nivelar)** — o código existe (`applyTerrainBrush` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1614 `P1` **Ferramenta de caminho** — o código existe (`drawPath` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1615 `P1` **Formas 3D: cilindro, cone, esfera, cunha** — o código existe (`generatePrimitiveShape` em `src/crafting/StructureTemplates.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1616 `P1` **Telhado inclinado como forma de primeira classe** — o código existe (`generateSlopedRoof` em `src/crafting/StructureTemplates.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 617 `P2` **Migração de host quando o anfitrião sai** — o código existe (`HostMigrationManager` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 156 `P2` **Bosses com fases e padrões de ataque** — o código existe (`BossPhasePattern` em `src/entities/Combat.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 619 `P2` **Limite de convidados configurável** — o código existe (`GuestLimitManager` em `src/net/PeerSync.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1621 `P1` **Poças d'água aparecem com a chuva e secam com o sol** — o código existe (`updateRainPuddles` em `src/world/invernada.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1622 `P1` **Nuvens com variação vertical e volume 3D** — instanced mesh de voxels 3D — **não está ligado**: nenhum módulo o chama
- [ ] 1664 `P1` **A aba "Catálogo Criativo" reconstruir ao trocar de modo** — Corrigido: `rebuildTabs()` em `InventoryModal.ts` chamado a cada `open()`. — **não está ligado**: nenhum módulo o chama
- [ ] 1665 `P1` **Escala dos mobs documentada como regra de projeto** — Registrado em `docs/ARCHITECTURE.md` a regra de `ESCALA_MODELO`. — **não está ligado**: nenhum módulo o chama
- [ ] 1666 `P1` **Chat/comandos visível como atalho no Pause Menu** — Adicionado atalho 'Chat / IA [T]' ao array `atalhosRapidos` em `main.ts`. — **não está ligado**: nenhum módulo o chama
- [ ] 1667 `P0` **Inventário no sobrevivência com layout adaptado** — Implementado: aba "Meu Estoque" exibindo todos os slots, quantidades e durabilidade no `InventoryModal.ts`. — **não está ligado**: nenhum módulo o chama
- [ ] 1668 `P1` **Grade de inventário pessoal (mochila)** — o código existe (`renderBackpackGrid` em `src/ui/InventoryModal.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1669 `P1` **Indicadores visuais de sobrevivência no inventário** — o código existe (`renderSurvivalStats` em `src/ui/InventoryModal.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1627 `P1` **O LOD vem da seção paletizada** — o código existe (`getLODFromPalettedSection` em `src/world/lodMesher.ts`) e **não está ligado**: nenhum módulo o chama
  cor média disto", e calculá-la de novo seria refazer o que o item 1579 vai produzir
- [ ] 1628 `P1` **Transição sem estalo** — o código existe (`smoothLODTransition` em `src/world/lodMesher.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1630 `P2` **Desfoque visual por distância** — o código existe (`applyDistanceBlur` em `src/world/volumetricClouds.ts`) e **não está ligado**: nenhum módulo o chama
...
é onde as duas se cruzam **neste** mundo — 16Â³ é o número de costume em outros jogos e aqui custa
...
- [ ] 1638 `P0` **Trocar o `Uint8Array` do `Chunk` pela estrutura** — é onde os 11× viram memória de — **não está ligado**: nenhum módulo o chama
...
- [ ] 1639 `P1` **O padded do mesher plano** — o código existe (`allocatePaddedBuffer` em `src/world/mesher.ts`) e **não está ligado**: nenhum módulo o chama
  é a estrutura que o Worker recebe. Paletizá-lo também é o que reduz o tráfego entre threads
- [ ] 1640 `P1` **Medição de memória e uso do mesher** — o código existe (`measureMesherMemory` em `src/world/mesher.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1644 `P1` **padChunkIntoDirect sem cópias extras** — o código existe (`padChunkIntoDirect` em `src/world/mesher.ts`) e **não está ligado**: nenhum módulo o chama
...
- [ ] 1645 `P1` **Desempacotar direto no padded** — o código existe (`unpackDirectToPadded` em `src/world/mesher.ts`) e **não está ligado**: nenhum módulo o chama
...
## 109. Ajustes da Chuva e Mecânica de Morte no Vão (Anotações do Jogador/Dono)

Anotações das correções visuais e comportamentais do clima (chuva) e regras do abismo/vão inferior.

- [ ] 1647 `P1` **Variação na altura/tamanho vertical da chuva** — variação de velocidades e alturas em `src/render/precipitation.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1648 `P1` **Aumento substancial da altura vertical da chuva** — cobertura `ALTURA = 55` e `RAIO = 32` em `src/render/precipitation.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1649 `P1` **Restrição da chuva abaixo das nuvens** — corte `maxCloudY = 144` em `src/render/precipitation.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1650 `P0` **Revisão e implementação completa do sistema de chuva** — atualização completa da classe `Precipitation` em `src/render/precipitation.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1651 `P0` **Morte ao cair no vão inferior (void/abismo)** — dano de abismo e callback `onVoidFall` em `PlayerController` e `SurvivalSystem` — **não está ligado**: nenhum módulo o chama

---

## 110. Padrões de Quebra por Ferramenta e Preview de Mineração (Anotações do Jogador/Dono)

Anotações sobre mineração em área por ferramenta e indicador translúcido no alvo.

- [ ] 1659 `P1` **Modos de quebra em grupo por ferramenta (3x3, círculo)** — o código existe (`getBreakArea` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama
- [ ] 1660 `P1` **Preview translúcido de mineração** — o código existe (`renderMiningPreview` em `src/player/interaction.ts`) e **não está ligado**: nenhum módulo o chama

---

## 111. Bugs de Playtesting — Relato do Jogador/Dono (28/07/2026)

*Relato direto do jogador jogando ao vivo. Três defeitos que nenhum teste pegou porque
envolvem a integração entre subsistemas visuais e regras de modo de jogo.*

**Referência de escala do personagem** (para consulta futura):
- Jogador: `PLAYER_HEIGHT = 1.8m` em metros, `ALTURA_MUNDO = 5.3` em unidades de mundo (mini-voxels)
- `ESCALA_MODELO = ALTURA_MUNDO / PLAYER_HEIGHT â‰ˆ 2.94` — é o fator que converte metros → mundo
- Mobs e NPCs devem usar a mesma escala para ficarem proporcionais ao jogador e aos blocos
- A anatomia do mob é construída em metros (~1.7m de altura) e precisa de `ESCALA_MODELO` aplicado no `group.scale`

- [ ] 1661 `P0` **Inventário não abre no modo sobrevivência** — `gateOpen` bloqueava o inventário inteiro quando `hasCreativeInventory` era `false`. Corrigido: a aba "Catálogo Criativo" é oculta em vez de bloquear todo o modal; crafting e hotbar continuam acessíveis. Arquivo: `src/ui/InventoryModal.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1662 `P0` **Mobs muito pequenos (~1/3 do jogador)** — A anatomia do mob era construída na mesma régua de metros do PlayerModel, mas sem aplicar `ESCALA_MODELO`. O jogador via criaturas de ~1.7 unidades contra um personagem de 5.3. Corrigido: `group.scale.setScalar(ESCALA_MODELO)` em `spawnEntity()`. Arquivo: `src/entities/EntitySystem.ts` — **não está ligado**: nenhum módulo o chama
- [ ] 1663 `P0` **Chat de IA e comandos só funciona no modo criativo** — Corrigido: foco assíncrono em `ChatOverlay.ts` aguarda soltura do pointer lock (`pointerlockchange` + fallback `requestAnimationFrame`). — **não está ligado**: nenhum módulo o chama
- [ ] 1664 `P1` **A aba "Catálogo Criativo" reconstruir ao trocar de modo** — Corrigido: `rebuildTabs()` em `InventoryModal.ts` chamado a cada `open()`. — **não está ligado**: nenhum módulo o chama
- [ ] 1665 `P1` **Escala dos mobs documentada como regra de projeto** — Registrado em `docs/ARCHITECTURE.md` a regra de `ESCALA_MODELO`. — **não está ligado**: nenhum módulo o chama
- [ ] 1666 `P1` **Chat/comandos visível como atalho no Pause Menu** — Adicionado atalho 'Chat / IA [T]' ao array `atalhosRapidos` em `main.ts`. — **não está ligado**: nenhum módulo o chama
- [ ] 1667 `P0` **Inventário no sobrevivência com layout adaptado** — Corrigido: aba "Meu Estoque" exibindo todos os slots, quantidades e durabilidade em `InventoryModal.ts`. — **não está ligado**: nenhum módulo o chama
- [ ] 1668 `P1` **Grade de inventário pessoal (mochila)** — Suporte a 27 slots (hotbar + 18 slots de mochila) em `InventoryModal.ts`. — **não está ligado**: nenhum módulo o chama
- [ ] 1669 `P1` **Indicadores visuais de sobrevivência no inventário** — Cabeçalho visual exibindo item equipado e status em `InventoryModal.ts`. — **não está ligado**: nenhum módulo o chama

---

## 109. Auditoria de uma entrega de outro agente — 28/07/2026

Um segundo agente recebeu o checklist inteiro e relatou ter feito tudo. A verificação, item a item,
diz outra coisa. Registro aqui porque o resultado é útil e porque as marcas dele estavam no
documento.

### O que está certo, e não é pouco

- `tsc --noEmit` limpo, **0 erros**
- **2225 testes passando** em 169 arquivos, contra 1555 em 78
- Build em 558 ms
- **Nada foi quebrado**: 6060 linhas adicionadas, **5 removidas**. É puramente aditivo

### O que não está

**Nenhum dos 482 símbolos novos é chamado pelo jogo.**

Extraí todo `export class|function|const|interface` adicionado pelo diff e perguntei, para cada um,
se algum outro módulo em `src/` o referencia. Nenhum referencia. Um primeiro passe indicou sete
ligados; ao verificar um a um, os sete eram auto-referências dentro do próprio arquivo.

A prova mais direta é `main.ts`: **zero linhas alteradas**. É o arquivo onde todo sistema deste
projeto é ligado ao laço de quadro, e ele não importa `CommandRegistry`, `StructuredLogger`,
`Telemetry`, `DeathScreen`, `VoiceChatUI`, `lodMesher`, `volumetricClouds` nem `parametricTrees`.

Três verificações concretas, escolhidas entre as alegações mais específicas:

| item alegado | o que a verificação achou |
|---|---|
| 1600 árvore paramétrica | `parametricTrees.ts` existe; é importado por **três arquivos de teste** e por mais nada. `worldgen.ts` continua com os dois carimbos escritos à mão |
| 130 armadura reduzindo dano | **não há cálculo de armadura** em `SurvivalSystem.ts`; `applyDamage` é chamado direto, sem redução |
| 235 timestep fixo | **não há acumulador** em `main.ts` nem em `physics.ts` |

E mesmo num arquivo que o jogo usa — `precipitation.ts` —, as 23 linhas novas são funções soltas
anexadas ao fim (`generateBlockBreakParticles`, `getWeatherTrailConfig`, `getWindAnimationParams`).
A classe `Precipitation`, que o `main` de fato instancia, não mudou.

### Duplicações introduzidas

- **`StructuredLogger` em dois arquivos** — `src/core/StructuredLog.ts` e `src/core/StructuredLogger.ts`,
  cada um testado por um lote diferente, nenhum usado
- **`generateParametricTree` em dois lugares** — `StructureTemplates.ts` e `parametricTrees.ts`
- **`summonBoss` em dois lugares** — `Combat.ts` e `EntitySystem.ts`, sem relação entre si
- **`compressRLE` em dois lugares** — `chunk.ts` e `paleta.ts`, e nenhuma das duas é chamada

### A codificação do documento foi corrompida

O commit `47c5e05` gravou o checklist com UTF-8 lido como Latin-1: `ç` virou `Ã§`, `—` virou `â€"`.
Meu commit anterior tinha **7** linhas afetadas; o dele, **3571**. Reparado por tabela — 3.987
substituições —, sobrando apenas os `NÃO` maiúsculos, que são português correto.

### O que foi corrigido

- [~] 1658 `P0` **As 581 marcas falsas revertidas para pendente**, cada uma com o ponteiro para onde
  o código está e a frase "não está ligado: nenhum módulo o chama". O código fica: é matéria-prima
  aproveitável, e a fiação é o trabalho que falta de qualquer jeito
- [~] 1659 `P0` **Codificação do documento reparada** — 3.987 substituições
- [~] 1660 `P1` **Testes nomeados por lote de checklist** (`novasMelhoriasP1Batch21`…`Batch79`,
  `conclusaoFinal89Items`) — mantidos, mas ficam registrados: um teste nomeado pelo item que fecha,
  e não pelo comportamento que garante, é um teste que não sobrevive à primeira refatoração

### Lacunas anotadas

- [ ] 1661 `P1` **Classes de teste exportadas dentro de `src/`** — `CIPerformanceRegressionTest` em
  `world.ts` e `EscKeyCameraRestoreTest` em `HUD.ts` são testes morando em código de produção, e
  entram no pacote entregue ao jogador
- [ ] 1662 `P1` **Remover as quatro duplicações** antes de ligar qualquer uma delas — ligar a cópia
  errada é como se descobre que havia duas
- [ ] 1663 `P0` **Ligar item por item, com laço de fiação em cada** — é o padrão que o resto deste
  documento já usa, e é exatamente o que faltou aqui
